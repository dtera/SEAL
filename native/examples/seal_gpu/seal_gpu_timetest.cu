...
#include <sys/time.h>
#include <vector>
#include "seal_gpu/seal_cuda.cuh"
...
namespace seal_gpu_test
{
    ...
    public:
        ...
        void printTimer(std::map<std::string, double> r)
        {
            for (auto &p : r)
            {
                ...
                std::cout << std::setw(10) << std::right << std::fixed << std::setprecision(3) << p.second << std::endl;
            }
        }
    ...
    public:
        TimeTestCKKS(
            size_t polyModulusDegree, vector<int> qs, int dataBound = 1 << 6,
    ...
    public:
        TimeTestBFVBGV(
            bool bgv, size_t polyModulusDegree, uint64_t plainModulusBitSize, vector<int> qs, int dataBound = 1 << 6)
        {
            KernelProvider::initialize();
            slotCount = polyModulusDegree / 2;
            this->dataBound = dataBound;
...
int main()
{
    ...
    std::cout << "----- BFV -----\n";
    seal_gpu_test::TimeTestBFVBGV test2(false, 16384, 59, { 60, 40, 40, 40, 40, 60 });
    test2.testAll();

    std::cout << "----- BGV -----\n";
    seal_gpu_test::TimeTestBFVBGV test3(true, 16384, 20, { 60, 40, 40, 40, 40, 60 });
    test3.testAll();
    return 0;
}

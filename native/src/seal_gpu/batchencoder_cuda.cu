...
namespace seal_gpu
{
    ...
    __global__ void gEncodeUnsignedUtil(
        const uint64_t *values_matrix, size_t slots, size_t values_matrix_size, uint64_t *destination,
        const uint64_t *matrix_reps_index_map)
    {
        GET_INDEX_COND_RETURN(slots);
        destination[matrix_reps_index_map[gindex]] = (gindex < values_matrix_size) ? values_matrix[gindex] : 0;
    }
    ...
    void BatchEncoderCuda::encode(const vector<uint64_t> &values_matrix, PlaintextCuda &destination) const
    {
        ...
        (values_matrix_device.get(), slots_, values_matrix_size, destination.data(), matrix_reps_index_map_.get());

        kernel_util::kInverseNttNegacyclicHarvey(
            destination.data(), 1, 1, getPowerOfTwo(slots_), context_data.plainNTTTables());
    }

    __global__ void gEncodeSignedUtil(
        const int64_t *values_matrix, size_t slots, size_t values_matrix_size, uint64_t *destination,
        const uint64_t *matrix_reps_index_map, uint64_t modulus)
    {
        GET_INDEX_COND_RETURN(slots);
        destination[matrix_reps_index_map[gindex]] =
            (gindex < values_matrix_size)
                ? ((values_matrix[gindex] < 0) ? (modulus + static_cast<uint64_t>(values_matrix[gindex]))
    ...
    __global__ void gDecodeUnsignedUtil(
        const uint64_t *temp_dest, size_t slots, uint64_t *destination, const uint64_t *matrix_reps_index_map)
    {
        GET_INDEX_COND_RETURN(slots);
    ...
    void BatchEncoderCuda::decode(const PlaintextCuda &plain, vector<uint64_t> &destination) const
    {
        ...
        size_t plain_coeff_count = std::min(plain.coeffCount(), slots_);
        ...
        (temp_dest.get(), slots_, destination_device.get(), matrix_reps_index_map_.get());

        KernelProvider::retrieve(destination.data(), destination_device.get(), slots_);
    }

    __global__ void gDecodeSignedUtil(
        const uint64_t *temp_dest, size_t slots, int64_t *destination, const uint64_t *matrix_reps_index_map,
        uint64_t modulus)
    {
        GET_INDEX_COND_RETURN(slots);
    ...
    void BatchEncoderCuda::decode(const PlaintextCuda &plain, vector<int64_t> &destination) const
    {
        ...
        (temp_dest.get(), slots_, destination_device.get(), matrix_reps_index_map_.get(), modulus);

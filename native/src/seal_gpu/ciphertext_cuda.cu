...
namespace seal_gpu
{
    ...
    void CiphertextCuda::saveTerms(
        std::ostream &stream, EvaluatorCuda &evaluator, const std::vector<size_t> &termIds) const
    {
        savet(stream, &parms_id_);
        savet(stream, &is_ntt_form_);
    ...
    void CiphertextCuda::load(std::istream &stream)
    {
        ...
        loadt(stream, &terms);
        if (terms)
            throw std::invalid_argument("Trying to load a termed ciphertext, but indices is not specified");
        if (seed == 0)
        {
            size_t dataSize;
    ...
    void CiphertextCuda::loadTerms(std::istream &stream, EvaluatorCuda &evaluator, const std::vector<size_t> &termIds)
    {
        ...
        loadt(stream, &terms);
        if (!terms)
            throw std::invalid_argument("Trying to load a normal ciphertext, but term indices is specified");
    ...
    void CiphertextCuda::load(std::istream &stream, const SEALContextCuda &context)
    {
        ...
        else
        {
            ...
            util::sampler::kSamplePolyUniform(
                curandStates.get(), modulus.size(), poly_modulus_degree_, modulus, data(1));

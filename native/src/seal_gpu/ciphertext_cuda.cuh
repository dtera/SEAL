...
namespace seal_gpu
{
    ...
    public:
        ...
        CiphertextCuda(const Ciphertext &host)
            : parms_id_(host.parms_id_), is_ntt_form_(host.is_ntt_form_), size_(host.size_),
              poly_modulus_degree_(host.poly_modulus_degree_), coeff_modulus_size_(host.coeff_modulus_size_),
              scale_(host.scale_), correction_factor_(host.correction_factor_), data_(host.data_)
        {}
        ...
        inline Ciphertext toHost() const
        {
            return cpu();
        }
        ...
        inline util::DevicePointer<uint64_t> data()
        {
        ...
        inline util::DeviceDynamicArray<ct_coeff_type> &dynArray()
        {
            return data_;
        }
        ...
        explicit CiphertextCuda(const SEALContextCuda &context, ParmsID parms_id) : data_()
        {
            // Allocate memory but don't resize
            reserve(context, parms_id, 2);
        }

        explicit CiphertextCuda(const SEALContextCuda &context, ParmsID parms_id, std::size_t size_capacity) : data_()
        {
            // Allocate memory but don't resize
        ...
        inline std::uint64_t seed() const noexcept
        {
            return seed_;
        }
        ...
        static inline CiphertextCuda fromMembers(
            ParmsID parms_id, std::size_t size, std::size_t coeff_modulus_size, std::size_t poly_modulus_degree,
            bool is_ntt_form, double scale, std::uint64_t correction_factor,
    ...
    private:
        void reserveInternal(std::size_t size_capacity, std::size_t poly_modulus_degree, std::size_t coeff_modulus_size)
        {
    ...
    public:
        ...
        inline util::DevicePointer<uint64_t> c1()
        {
        ...
        inline util::DevicePointer<uint64_t> c0()
        {
        ...
        inline static LWECiphertextCuda fromMembers(
            ParmsID parms_id, std::size_t poly_modulus_degree, std::size_t coeff_modulus_size, double scale,
            std::uint64_t correction_factor, util::DeviceDynamicArray<ct_coeff_type> &&c1,

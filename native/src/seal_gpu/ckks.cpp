...
namespace seal_gpu
{
                ...
                if (is_negative)
                {
                    for (std::size_t j = 0; j < coeff_modulus_size; j++)
                    {
                        destination[i + (j * coeff_count)] =
                            util::negateUintMod(util::barrettReduce64(coeffu, coeff_modulus[j]), coeff_modulus[j]);
                    }
                }
                else
                {
        ...
        else if (max_coeff_bit_count <= 128)
        {
            for (std::size_t i = 0; i < n; i++)
            {
                ...
                if (is_negative)
                {
                    ...
                    {
                        destination[i + (j * coeff_count)] =
                            util::negateUintMod(util::barrettReduce128(coeffu, coeff_modulus[j]), coeff_modulus[j]);
                    }
                }
                else
                {
    ...
    void CKKSEncoder::encodeInternal(double value, ParmsID parms_id, double scale, Plaintext &destination)
    {
        // Verify parameters.
        auto context_data_ptr = context_.getContextData(parms_id);
        if (!context_data_ptr)
        ...
        if (coeff_bit_count <= 64)
        {
            ...
            else
            {
                ...
                {
                    fill_n(
                        destination.data() + (j * coeff_count), coeff_count, barrettReduce64(coeffu, coeff_modulus[j]));
                }
            }
        }

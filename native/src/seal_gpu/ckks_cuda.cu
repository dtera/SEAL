...
namespace seal_gpu
{
    ...
    CKKSEncoderCuda::CKKSEncoderCuda(const SEALContextCuda &context) : context_(context)
    {
    ...
    __device__ inline void dSetComplex(double *array, size_t index, double real, double imag)
    {
        array[index * 2] = real;
    ...
    __global__ void gEncodeInternalSetConjValues(
        const double *copied_values, size_t values_size, size_t slots, double *conj_values,
    ...
    __global__ void gFftTransferFromRevLayered(
        size_t layer, double *operand, size_t poly_modulus_degree_power, const double *roots)
    {
    ...
    void kFftTransferFromRev(
        complex<double> *operand, size_t poly_modulus_degree_power, const complex<double> *roots, double fix = 1)
    {
    ...
    __global__ void gMaxReal(double *complexes, size_t n_sqrt, size_t n, double *out)
    {
        ...
        FOR_N(i, n_sqrt)
        {
            ...
            if (id >= n)
                break;
            if (fabs(complexes[id * 2]) > m)
                m = fabs(complexes[id * 2]);
        }
        out[gindex] = m;
    }
    ...
    __global__ void gEncodeInternalComplexArrayUtilA(
        double *conj_values, size_t n, size_t coeff_modulus_size, const Modulus *coeff_modulus, uint64_t *destination,
    ...
    __global__ void gEncodeInternalComplexArrayUtilC(
        double *conj_values, size_t n, size_t coeff_modulus_size, const Modulus *coeff_modulus, uint64_t *coeffu_array,
        bool is_complex = true)
    {
        GET_INDEX_COND_RETURN(n);
        double two_pow_64 = pow(2.0, 64);
    ...
    __global__ void gEncodeInternalComplexArrayUtilD(
        const double *conj_values, const uint64_t *coeffu_array, size_t n, size_t coeff_modulus_size,
        const Modulus *coeff_modulus, uint64_t *destination, bool is_complex = true)
    {
        GET_INDEX_COND_RETURN(n);
        double coeffd = round(conj_values[gindex * (is_complex ? 2 : 1)]);
        bool is_negative = coeffd < 0;
    ...
    __global__ void gCopyMulScalar(const double *from, double *to, size_t n, double scalar)
    {
        GET_INDEX_COND_RETURN(n);
    ...
    void CKKSEncoderCuda::encodeInternal(
        const std::complex<double> *values, std::size_t values_size, ParmsID parms_id, double scale,
        PlaintextCuda &destination)
    {
...

        {
            ...
            (reinterpret_cast<const double *>(copied_values.get()), values_size, slots_,
             reinterpret_cast<double *>(conj_values.get()), matrix_reps_index_map_.get());
        }
        ...
        kFftTransferFromRev(conj_values.get(), util::getPowerOfTwo(n), inv_root_powers_.get(), fix);
        ...
        auto max_coeff_array = util::DeviceArray<double>(n_sqrt);
        {
            ...
            (reinterpret_cast<double *>(conj_values.get()), n_sqrt, n, max_coeff_array.get());
        }
        ...
        if (max_coeff_bit_count <= 64)
        {
            ...
            (reinterpret_cast<double *>(conj_values.get()), n, coeff_modulus_size, coeff_modulus.get(),
             destination.data());
        }
        ...
        else
        {
            ...
            gEncodeInternalComplexArrayUtilD<<<block_count, 256>>>(
                reinterpret_cast<double *>(conj_values.get()), coeffu_array.get(), n, coeff_modulus_size,
    ...
    void CKKSEncoderCuda::encodePolynomialInternal(
        const double *values, std::size_t values_size, ParmsID parms_id, double scale, PlaintextCuda &destination)
    {
        // Verify parameters.
        auto context_data_ptr = context_.getContextData(parms_id);
        if (!context_data_ptr)
    ...
    __global__ void gEncodeInternalDoubleUtilA(
        double coeffd, size_t n, size_t coeff_modulus_size, const Modulus *coeff_modulus, uint64_t *destination)
    {
        GET_INDEX_COND_RETURN(n);
        bool is_negative = coeffd < 0;
        uint64_t coeffu = static_cast<uint64_t>(abs(coeffd));
        FOR_N(j, coeff_modulus_size)
        {
            uint64_t fill = is_negative ? kernel_util::dNegateUintMod(
                                              kernel_util::dBarrettReduce64(coeffu, coeff_modulus[j]), coeff_modulus[j])
    ...
    __global__ void gEncodeInternalDoubleUtilC(
        const uint64_t *coeffu, bool is_negative, size_t n, size_t coeff_modulus_size, const Modulus *coeff_modulus,
        uint64_t *destination)
    {
        GET_INDEX_COND_RETURN(n);
        FOR_N(j, coeff_modulus_size)
        {
            uint64_t fill = is_negative ? kernel_util::dNegateUintMod(coeffu[j], coeff_modulus[j]) : coeffu[j];
            destination[gindex + j * n] = fill;
        }
    }

    void CKKSEncoderCuda::encodeInternal(double value, ParmsID parms_id, double scale, PlaintextCuda &destination)
    {
        // Verify parameters.
        auto context_data_ptr = context_.getContextData(parms_id);
        if (!context_data_ptr)
        ...
        if (coeff_bit_count <= 64)
        {
            ...
            (coeffd, coeff_count, coeff_modulus_size, coeff_modulus.get(), destination.data());
        }
        ...
        else
        {
            ...
            (coeffu_dev.get(), is_negative, coeff_count, coeff_modulus_size, coeff_modulus.get(), destination.data());
        }
    ...
    __global__ void gEncodeInternalInt64(
        int64_t value, size_t n, size_t coeff_modulus_size, const Modulus *coeff_modulus, uint64_t *destination)
    {
        GET_INDEX_COND_RETURN(n);
        FOR_N(j, coeff_modulus_size)
        {
    ...
    void CKKSEncoderCuda::encodeInternal(int64_t value, ParmsID parms_id, PlaintextCuda &destination)
    {
        ...
        (value, coeff_count, coeff_modulus_size, coeff_modulus.get(), destination.data());
    ...
    __global__ void gDecodeInternal(
        const uint64_t *plain_copy, size_t coeff_count, size_t coeff_modulus_size, const uint64_t *decryption_modulus,
        const uint64_t *upper_half_threshold, double inv_scale, double *res)
    {
        GET_INDEX_COND_RETURN(coeff_count);
        size_t i = gindex;
        bool greater = kernel_util::dCompareUint(
                           plain_copy + i * coeff_modulus_size, upper_half_threshold, coeff_modulus_size) >= 0;
        double two_pow_64 = pow(2.0, 64);
    ...
    void kFftTransferToRevLayered(
        size_t layer, complex<double> *operand, size_t poly_modulus_degree_power, const complex<double> *roots)
    {
        std::size_t n = size_t(1) << poly_modulus_degree_power;
    ...
    void kFftTransferToRev(complex<double> *operand, size_t poly_modulus_degree_power, const complex<double> *roots)
    {
        std::size_t m = 1;
        std::size_t layer = 0;
        std::size_t n = size_t(1) << poly_modulus_degree_power;
    ...
    __global__ void gDecodeInternalSetConjValues(
        const double *values, size_t slots, double *destination, uint64_t *matrix_reps_index_map)
    {
        GET_INDEX_COND_RETURN(slots);
    ...
    void CKKSEncoderCuda::decodeInternal(const PlaintextCuda &plain, std::complex<double> *destination)
    {
        ...
        // Check that scale is positive and not too large
        if (plain.scale() <= 0 || (static_cast<int>(log2(plain.scale())) >= context_data.totalCoeffModulusBitCount()))
        {
            throw std::invalid_argument("scale out of bounds");
        }
...

        {
            ...
            (plain_copy.get(), coeff_count, coeff_modulus_size, decryption_modulus.get(), upper_half_threshold.get(),
             inv_scale, reinterpret_cast<double *>(res.get()));
        }
...

        {
            ...
            (reinterpret_cast<const double *>(res.get()), slots_, reinterpret_cast<double *>(destination_device.get()),
    ...
    void CKKSEncoderCuda::decodePolynomialInternal(const PlaintextCuda &plain, double *destination)
    {
...

        {
            ...
            (reinterpret_cast<double *>(res.get()), destination_device.get(), coeff_count);
        }

...
namespace seal_gpu
{
    ...
    SEALContext::ContextData SEALContext::validate(EncryptionParameters parms)
    {
        ...
        multiplyManyUint64(coeff_modulus_values.get(), coeff_modulus_size, context_data.total_coeff_modulus_.get());
        ...
        if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv)
        {
            ...
            auto wide_plain_modulus(
                duplicateUintIfNeeded(plain_modulus.data(), plain_modulus.uint64Count(), coeff_modulus_size, false));
            divideUint(
    ...
    SEALContext::SEALContext(EncryptionParameters parms, bool expand_mod_chain, SecurityLevel sec_level)
        : sec_level_(sec_level)
    {
        ...
        context_data_map_.emplace(
            make_pair(parms.parmsID(), make_shared<const ContextData>(std::move(first_context_data))));

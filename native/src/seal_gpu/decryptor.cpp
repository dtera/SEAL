...
namespace seal_gpu
{
    namespace
    {
        void polyInftyNormCoeffmod(
            ConstHostPointer<uint64_t> poly, size_t coeff_uint64_count, size_t coeff_count, const uint64_t *modulus,
            uint64_t *result)
        {
    ...
    void Decryptor::dotProductCtSkArray(const Ciphertext &encrypted, HostPointer<uint64_t> destination)
    {
        ...
        if (encrypted_size == 2)
        {
            ...
            else
            {
                for (size_t i = 0; i < coeff_modulus_size; i++)
                {
                    ...
                    dyadicProductCoeffmod(
                        destination + d, secret_key_array + d, coeff_count, coeff_modulus[i], destination + d);
                    // Transform back
        ...
        else
        {
            ...
            {
                dyadicProductCoeffmod(
                    encrypted_copy + i * poly_coeff_count, secret_key_array_ + i * key_poly_coeff_count,
            ...
            {
                addPolyCoeffmod(
                    destination, encrypted_copy + i * poly_coeff_count, coeff_modulus_size, coeff_count,
                    &coeff_modulus[0], destination);
            }
            ...
            addPolyCoeffmod(
                destination, encrypted.data(), coeff_modulus_size, coeff_count, &coeff_modulus[0], destination);
        }
    }
    ...
    int Decryptor::invariantNoiseBudget(const Ciphertext &encrypted)
    {
        ...
        {
            multiplyPolyScalarCoeffmod(
                noise_poly.asPointer(), coeff_modulus_size, coeff_count, plain_modulus.value(), &coeff_modulus[0],
        ...
        // coeff_modulus_size);
        polyInftyNormCoeffmod(
            noise_poly.get(), coeff_modulus_size, coeff_count, context_data.totalCoeffModulus(), norm.get());
        ...
        int bit_count_diff =
            context_data.totalCoeffModulusBitCount() - getSignificantBitCountUint(norm.get(), coeff_modulus_size) - 1;

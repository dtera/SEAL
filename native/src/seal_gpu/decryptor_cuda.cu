...
namespace seal_gpu
{
    ...
    void DecryptorCuda::bgvDecrypt(const CiphertextCuda &encrypted, PlaintextCuda &destination)
    {
        ...
        if (encrypted.correctionFactor() != 1)
        {
            ...
            kernel_util::kMultiplyPolyScalarCoeffmod(
                destination.data(), 1, 1, coeff_count, fix, parms.plainModulusCuda(), destination.data());
        }
    ...
    void DecryptorCuda::computeSecretKeyArray(size_t max_power)
    {
        ...
        kernel_util::kSetPolyArray(
            secret_key_array_.get(), old_size, coeff_count, coeff_modulus_size, secret_key_array.get());
        ...
        for (size_t i = 0; i < new_size - old_size; i++)
        {
            kernel_util::kDyadicProductCoeffmod(
                secret_key_array.asPointer() + (old_size - 1 + i) * poly_coeff_count, secret_key_array.asPointer(), 1,
    ...
    void DecryptorCuda::dotProductCtSkArray(const CiphertextCuda &encrypted, DevicePointer<uint64_t> destination)
    {
        ...
        if (encrypted_size == 2)
        {
            ...
            {
                kernel_util::kDyadicProductCoeffmod(
                    c1, secret_key_array, 1, coeff_modulus_size, coeff_count, coeff_modulus, destination);
                kernel_util::kAddPolyCoeffmod(
                    destination, c0, 1, coeff_modulus_size, coeff_count, coeff_modulus, destination);
            }
            else
            {
                ...
                kernel_util::kDyadicProductCoeffmod(
                    destination, secret_key_array, 1, coeff_modulus_size, coeff_count, coeff_modulus, destination);
        ...
        else
        {
            ...
            kernel_util::kSetPolyArray(
                encrypted.data(1), encrypted_size - 1, coeff_modulus_size, coeff_count, encrypted_copy.get());
            if (!isNttForm)
            {
                kernel_util::kNttNegacyclicHarveyLazy(
                    encrypted_copy, encrypted_size - 1, coeff_modulus_size, coeff_power, ntt_tables);
            }
            for (size_t i = 0; i < encrypted_size - 1; i++)
            {
                kernel_util::kDyadicProductCoeffmod(
                    encrypted_copy + i * poly_coeff_count, secret_key_array_ + i * key_poly_coeff_count, 1,
                    coeff_modulus_size, coeff_count, coeff_modulus, encrypted_copy + i * poly_coeff_count);
            }
            ...
            {
                kernel_util::kAddPolyCoeffmod(
                    destination, encrypted_copy + i * poly_coeff_count, 1, coeff_modulus_size, coeff_count,
            ...
            kernel_util::kAddPolyCoeffmod(
                destination, encrypted.data(), 1, coeff_modulus_size, coeff_count, coeff_modulus, destination);
        }
    }

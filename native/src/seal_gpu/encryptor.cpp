...
namespace seal_gpu
{
    ...
    void Encryptor::encryptZeroInternal(ParmsID parms_id, bool is_asymmetric, Ciphertext &destination) const
    {
        // Verify parameters.
        ...
        if (is_asymmetric)
        {
            ...
            if (prev_context_data_ptr)
            {
                ...
                for (size_t i = 0; i < temp.size(); i++)
                {
                    ...
                    setPoly(
                        temp.data() + i * (poly_element_count + coeff_count), coeff_count, coeff_modulus_size,
    ...
    void Encryptor::encryptInternal(const Plaintext &plain, bool is_asymmetric, Ciphertext &destination) const
    {
        ...
        else if (scheme == SchemeType::ckks)
        {
            ...
            addPolyCoeffmod(
                destination_iter, plain_iter, coeff_modulus_size, coeff_count, &coeff_modulus[0], destination_iter);

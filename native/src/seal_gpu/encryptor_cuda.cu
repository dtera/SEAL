...
namespace seal_gpu
{
    ...
    EncryptorCuda::EncryptorCuda(
        const SEALContextCuda &context, const PublicKeyCuda &public_key, const SecretKeyCuda &secret_key)
        : context_(context)
    {
        setPublicKey(public_key);
        setSecretKey(secret_key);
    ...
    void EncryptorCuda::encryptZeroInternal(ParmsID parms_id, bool is_asymmetric, CiphertextCuda &destination) const
    {
        // Verify parameters.
        ...
        if (is_asymmetric)
        {
            ...
            if (prev_context_data_ptr)
            {
                ...
                for (size_t i = 0; i < temp.size(); i++)
                {
                    ...
                    kernel_util::kSetPolyArray(
                        temp.data() + i * (poly_element_count + coeff_count), 1, coeff_modulus_size, coeff_count,
            ...
            else
            {
                ...
                util::encryptZeroAsymmetric(
                    public_key_, context_, parms_id, isNttForm, destination, curandStates.asPointer());
            }
        }
        else
        {
            // Does not require modulus switching
            util::encryptZeroSymmetric(
                secret_key_, context_, parms_id, isNttForm, destination, curandStates.asPointer());
        }
    }
        ...
        const PlaintextCuda &plain, bool is_asymmetric, CiphertextCuda &destination) const
    {
        ...
        else if (scheme == SchemeType::ckks)
        {
            ...
            kernel_util::kAddPolyCoeffmod(
                destination_iter, plain_iter, 1, coeff_modulus_size, coeff_count, coeff_modulus, destination_iter);

...
namespace seal_gpu
{
    namespace
    {
        ...
        inline bool isScaleWithinBounds(double scale, const SEALContext::ContextData &context_data) noexcept
        {
    ...
    void Evaluator::negateInplace(Ciphertext &encrypted) const
    {
        ...
        negatePolyCoeffmod(
            encrypted.data(), encrypted_size, coeff_modulus.size(), parms.polyModulusDegree(), &coeff_modulus[0],
            encrypted.data());
    }
    ...
    void Evaluator::addInplace(Ciphertext &encrypted1, const Ciphertext &encrypted2) const
    {
        ...
        else
        {
            ...
            // Add ciphertexts
            addPolyCoeffmod(
                encrypted1.data(), encrypted2.data(), min_count, coeff_modulus_size, coeff_count, &coeff_modulus[0],
                encrypted1.data());
    ...
    void Evaluator::subInplace(Ciphertext &encrypted1, const Ciphertext &encrypted2) const
    {
        ...
        else
        {
            ...
            subPolyCoeffmod(
                encrypted1.data(), encrypted2.data(), min_count, coeff_modulus_size, coeff_count, coeff_modulus.data(),
                encrypted1.data());
            ...
            {
                negatePolyCoeffmod(
                    encrypted2.data(min_count), encrypted2_size - min_count, coeff_modulus_size, coeff_count,
                    coeff_modulus.data(), encrypted1.data(min_count));
            }
        }
    }
    ...
    void Evaluator::bfvMultiply(Ciphertext &encrypted1, const Ciphertext &encrypted2) const
    {
        ...
        for (size_t i = 0; i < encrypted1_size; i++)
        {
            ...
            nttNegacyclicHarveyLazy(
                encrypted1_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size, base_Bsk_ntt_tables);
        }
        ...
        for (size_t i = 0; i < encrypted2_size; i++)
        {
            ...
            nttNegacyclicHarveyLazy(
                encrypted2_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size, base_Bsk_ntt_tables);
        }
                ...
                for (size_t j = 0; j < steps; j++)
                {
                    for (size_t k = 0; k < base_q_size; k++)
                    {
                        ...
                        dyadicProductCoeffmod(
                            ...
                            shifted_reversed_in2_iter - j * d + k * coeff_count, coeff_count, base_q[k],
                            temp.asPointer());
                        // std::cout << "first of temp: " << temp[0] << std::endl;
                        addPolyCoeffmod(
                            temp.asPointer(), shifted_out_iter + k * coeff_count, coeff_count, base_q[k],
            ...
            }
            {
                ...
                for (size_t j = 0; j < steps; j++)
                {
                    for (size_t k = 0; k < base_Bsk_size; k++)
                    {
                        ...
                        dyadicProductCoeffmod(
                            shifted_in1_iter + j * d + k * coeff_count,
                            shifted_reversed_in2_iter - j * d + k * coeff_count, coeff_count, base_Bsk[k],
                            temp.asPointer());
                        addPolyCoeffmod(
                            temp.asPointer(), shifted_out_iter + k * coeff_count, coeff_count, base_Bsk[k],
                            shifted_out_iter + k * coeff_count);
                    }
                }
            }
        ...
        for (size_t i = 0; i < dest_size; i++)
        {
            ...
            multiplyPolyScalarCoeffmod(
                temp_dest_q + i * coeff_count * base_q_size, base_q_size, coeff_count, plain_modulus, base_q,
                temp_q_Bsk.asPointer());

            multiplyPolyScalarCoeffmod(
                temp_dest_Bsk + i * coeff_count * base_Bsk_size, base_Bsk_size, coeff_count, plain_modulus, base_Bsk,
            ...
            for (size_t j = 0; j < steps; j++)
            {
                for (size_t k = 0; k < coeff_modulus_size; k++)
                {
                    ...
                    dyadicProductCoeffmod(
                        ...
                        shifted_reversed_encrypted2_iter - j * d + k * coeff_count, coeff_count, coeff_modulus[k],
                        prod.asPointer());
                    addPolyCoeffmod(
                        prod.asPointer(), temp + i * d + k * coeff_count, coeff_count, coeff_modulus[k],
    ...
    void Evaluator::bfvSquare(Ciphertext &encrypted) const
    {
        ...
        for (size_t i = 0; i < encrypted_size; i++)
        {
            ...
            // Lazy reduction
            nttNegacyclicHarveyLazy(
                encrypted_Bsk.get() + i * coeff_count * base_Bsk_size, base_Bsk_size, base_Bsk_ntt_tables);
        }
        ...
        // Compute c0^2
        dyadicProductCoeffmod(
            encrypted_q + 0 * d_q, encrypted_q + 0 * d_q, base_q_size, coeff_count, base_q, temp_dest_q + 0 * d_q);
        // Compute 2*c0*c1
        dyadicProductCoeffmod(
            encrypted_q + 0 * d_q, encrypted_q + 1 * d_q, base_q_size, coeff_count, base_q, temp_dest_q + 1 * d_q);
        addPolyCoeffmod(
            temp_dest_q + 1 * d_q, temp_dest_q + 1 * d_q, base_q_size, coeff_count, base_q, temp_dest_q + 1 * d_q);
        // Compute c1^2
        dyadicProductCoeffmod(
            encrypted_q + 1 * d_q, encrypted_q + 1 * d_q, base_q_size, coeff_count, base_q, temp_dest_q + 2 * d_q);

        // Compute c0^2
        dyadicProductCoeffmod(
            encrypted_Bsk + 0 * d_Bsk, encrypted_Bsk + 0 * d_Bsk, base_Bsk_size, coeff_count, base_Bsk,
            temp_dest_Bsk + 0 * d_Bsk);
        // Compute 2*c0*c1
        dyadicProductCoeffmod(
            encrypted_Bsk + 0 * d_Bsk, encrypted_Bsk + 1 * d_Bsk, base_Bsk_size, coeff_count, base_Bsk,
            temp_dest_Bsk + 1 * d_Bsk);
        addPolyCoeffmod(
            temp_dest_Bsk + 1 * d_Bsk, temp_dest_Bsk + 1 * d_Bsk, base_Bsk_size, coeff_count, base_Bsk,
            temp_dest_Bsk + 1 * d_Bsk);
        // Compute c1^2
        dyadicProductCoeffmod(
            encrypted_Bsk + 1 * d_Bsk, encrypted_Bsk + 1 * d_Bsk, base_Bsk_size, coeff_count, base_Bsk,
    ...
    void Evaluator::ckksSquare(Ciphertext &encrypted) const
    {
        ...
        // Compute c1^2
        dyadicProductCoeffmod(
            encrypted_iter + 1 * d, encrypted_iter + 1 * d, coeff_modulus_size, coeff_count, coeff_modulus,
            encrypted_iter + 2 * d);

        // Compute 2*c0*c1
        dyadicProductCoeffmod(
            encrypted_iter + 0 * d, encrypted_iter + 1 * d, coeff_modulus_size, coeff_count, coeff_modulus,
            encrypted_iter + 1 * d);
        addPolyCoeffmod(
            encrypted_iter + 1 * d, encrypted_iter + 1 * d, coeff_modulus_size, coeff_count, coeff_modulus,
            encrypted_iter + 1 * d);
        ...
        // Compute c0^2
        dyadicProductCoeffmod(
            encrypted_iter + 0 * d, encrypted_iter + 0 * d, coeff_modulus_size, coeff_count, coeff_modulus,
            encrypted_iter + 0 * d);
    ...
    void Evaluator::modSwitchScaleToNext(const Ciphertext &encrypted, Ciphertext &destination) const
    {
    ...
    void Evaluator::modSwitchDropToNext(const Ciphertext &encrypted, Ciphertext &destination) const
    {
        // Assuming at this point encrypted is already validated.
        auto context_data_ptr = context_.getContextData(encrypted.parmsID());
        if (context_data_ptr->parms().scheme() == SchemeType::ckks && !encrypted.isNttForm())
            ...
            for (size_t i = 0; i < encrypted_size; i++)
            {
                for (size_t j = 0; j < next_coeff_modulus_size; j++)
                {
                    setUint(
                        ...
                        temp.get() + i * coeff_count * next_coeff_modulus_size + j * coeff_count);
                }
            }
    ...
    void Evaluator::modSwitchToNext(const Ciphertext &encrypted, Ciphertext &destination) const
    {
        // Verify parameters.
        if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted))
        {
    ...
    void Evaluator::exponentiateInplace(Ciphertext &encrypted, uint64_t exponent, const RelinKeys &relin_keys) const
    {
        // Verify parameters.
        auto context_data_ptr = context_.getContextData(encrypted.parmsID());
        if (!context_data_ptr)
    ...
    void Evaluator::addPlainInplace(Ciphertext &encrypted, const Plaintext &plain) const
    {
        ...
        case SchemeType::ckks:
        {
            ...
            addPolyCoeffmod(
                encrypted_iter, plain_iter, coeff_modulus_size, coeff_count, coeff_modulus.data(), encrypted_iter);
            break;
        }
        ...
        */
            ...
            if (plain[mono_exponent] >= plain_upper_half_threshold)
            {
                if (!context_data.qualifiers().using_fast_plain_lift)
                {
                    ...
                    negacyclicMultiplyPolyMonoCoeffmod(
                        encrypted.data(), encrypted_size, coeff_modulus_size, coeff_count, temp.asPointer(),
                        mono_exponent, coeff_modulus.data(), encrypted.data());
                }
                else
                {
                    ...
                    // multiplication.
                    negacyclicMultiplyPolyMonoCoeffmod(
                        encrypted.data(), encrypted_size, coeff_modulus_size, coeff_count, plain[mono_exponent],
                        mono_exponent, coeff_modulus.data(), encrypted.data());
                }
            }
            else
        ...
        if (!context_data.qualifiers().using_fast_plain_lift)
        {
            ...
            for (size_t i = 0; i < plain_coeff_count; i++)
            {
                ...
                {
                    addUint(
                        plain_upper_half_increment, coeff_modulus_size, plain_value,
        ...
        else
        {
            ...
            for (size_t i = 0; i < coeff_modulus_size; i++)
            {
                ...
                for (size_t j = 0; j < plain_coeff_count; j++)
                {
                    ...
                    // [&](auto J) {
                    temp[i * coeff_count + j] = plain.data()[j] >= plain_upper_half_threshold
    ...
    void Evaluator::multiplyPlainNtt(Ciphertext &encrypted_ntt, const Plaintext &plain_ntt) const
    {
        ...
        for (size_t i = 0; i < encrypted_ntt_size; i++)
        {
            ...
            dyadicProductCoeffmod(
                encrypted_ntt.data(i), plain_ntt_iter, coeff_modulus_size, coeff_count, coeff_modulus.data(),
    ...
    void Evaluator::applyGaloisInplace(Ciphertext &encrypted, uint32_t galois_elt, const GaloisKeys &galois_keys) const
    {
        // Verify parameters.
        if (!isMetadataValidFor(encrypted, context_) || !isBufferValid(encrypted))
        {
        ...
        if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv)
        {
            ...
            galois_tool->applyGalois(
                encrypted.data(0), coeff_modulus_size, galois_elt, coeff_modulus.data(), temp.asPointer());
            ...
            galois_tool->applyGalois(
                encrypted.data(1), coeff_modulus_size, galois_elt, coeff_modulus.data(), temp.asPointer());
        }
    ...
    void Evaluator::rotateInternal(Ciphertext &encrypted, int steps, const GaloisKeys &galois_keys) const
    {
        auto context_data_ptr = context_.getContextData(encrypted.parmsID());
        if (!context_data_ptr)
        {
    ...
    void Evaluator::switchKeyInplace(
        Ciphertext &encrypted, ConstHostPointer<uint64_t> target_iter, const KSwitchKeys &kswitch_keys,
            ...
            for (size_t j = 0; j < decomp_modulus_size; j++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        // printf("branch b - ");
                        moduloPolyCoeffs(
                            t_target.get() + j * coeff_count, coeff_count, key_modulus[key_index], t_ntt.get());
                    }
                ...
                for (size_t k = 0; k < key_component_count; k++)
                {
                    ...
                    if (!lazy_reduction_counter)
                    {
                        for (size_t l = 0; l < coeff_count; l++)
                        {
                            ...
                            multiplyUint64(
                                t_operand[l], key_vector[j].data().data(k)[key_index * coeff_count + l], qword);
            ...
            for (size_t k = 0; k < key_component_count; k++)
            {
                ...
                else
                {
                    ...
                    for (size_t l = 0; l < coeff_count; l++)
                    {
                        // std::cout << "*get<0>(L): " << accumulator[l * 2] << std::endl;
                        t_poly_prod_iter[k * coeff_count * rns_modulus_size + l] =
                            barrettReduce128(accumulator.get() + l * 2, key_modulus[key_index]);
                    }
                }
            }
        }
        ...
        for (size_t i = 0; i < key_component_count; i++)
        {
                ...
                for (size_t j = 0; j < decomp_modulus_size; j++)
                {
                    ...
                    for (size_t k = 0; k < coeff_count; k++)
                    {
                        t_poly_prod[t_poly_prod_index + k] =
                            t_poly_prod[t_poly_prod_index + k] + Lqi - (delta[k] + c_mod_qi[k]);
                    }
                    ...
                    multiplyPolyScalarCoeffmod(
                        t_poly_prod + t_poly_prod_index, coeff_count, modswitch_factors[j], key_modulus[j],
                        t_poly_prod + t_poly_prod_index);

                    addPolyCoeffmod(
                        t_poly_prod + t_poly_prod_index, encrypted.data(i) + j * coeff_count, coeff_count,
            ...
            else
            {
                ...
                for (size_t j = 0; j < decomp_modulus_size; j++)
                {
                    ...
                    multiplyPolyScalarCoeffmod(
                        t_poly_prod_ptr, coeff_count, modswitch_factors[j], key_modulus[j], t_poly_prod_ptr);
                    addPolyCoeffmod(
                        t_poly_prod_ptr, encrypted.data(i) + j * coeff_count, coeff_count, key_modulus[j],

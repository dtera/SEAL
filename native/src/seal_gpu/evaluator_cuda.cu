...
namespace seal_gpu
{
    ...
    namespace
    {
        ...
        inline bool isScaleWithinBounds(double scale, const SEALContextCuda::ContextDataCuda &context_data) noexcept
        {
        ...
        inline auto balanceCorrectionFactors(uint64_t factor1, uint64_t factor2, const Modulus &plain_modulus)
    ...
    void EvaluatorCuda::negateInplace(CiphertextCuda &encrypted) const
    {
        ...
        kernel_util::kNegatePolyCoeffmod(
            encrypted.data(), encrypted_size, coeff_modulus.size(), parms.polyModulusDegree(), coeff_modulus,
            encrypted.data());
    }
    ...
    void EvaluatorCuda::addInplace(CiphertextCuda &encrypted1, const CiphertextCuda &encrypted2) const
    {
        ...
        if (encrypted1.correctionFactor() != encrypted2.correctionFactor())
        {
            ...
            auto factors =
                balanceCorrectionFactors(encrypted1.correctionFactor(), encrypted2.correctionFactor(), plain_modulus);
        ...
        else
        {
            ...
            kernel_util::kAddPolyCoeffmod(
                encrypted1.data(), encrypted2.data(), min_count, coeff_modulus_size, coeff_count,
                coeff_modulus.asPointer(), encrypted1.data());
    ...
    void EvaluatorCuda::bfvMultiply(CiphertextCuda &encrypted1, const CiphertextCuda &encrypted2) const
    {
        ...
        kernel_util::kNttNegacyclicHarveyLazy(
            encrypted1_q.get(), encrypted1_size, base_q_size, coeff_power, base_q_ntt_tables);
        ...
        kernel_util::kNttNegacyclicHarveyLazy(
            encrypted1_Bsk.get(), encrypted1_size, base_Bsk_size, coeff_power, base_Bsk_ntt_tables);
        ...
        kernel_util::kNttNegacyclicHarveyLazy(
            encrypted2_q.get(), encrypted2_size, base_q_size, coeff_power, base_q_ntt_tables);
        ...
        kernel_util::kNttNegacyclicHarveyLazy(
            encrypted2_Bsk.get(), encrypted2_size, base_Bsk_size, coeff_power, base_Bsk_ntt_tables);
        ...
        for (size_t i = 0; i < dest_size; i++)
        {
...

            {
                ...
                kernel_util::kDyadicConvolutionCoeffmod(
                    shifted_in1_iter, shifted_reversed_in2_iter, steps, base_q_size, coeff_count, base_q,
                    shifted_out_iter);
            }
            {
                ...
                kernel_util::kDyadicConvolutionCoeffmod(
                    shifted_in1_iter, shifted_reversed_in2_iter, steps, base_Bsk_size, coeff_count, base_Bsk,
                    shifted_out_iter);
            }
        }

        kernel_util::kInverseNttNegacyclicHarveyLazy(
            temp_dest_q.get(), dest_size, base_q_size, coeff_power, base_q_ntt_tables);
        kernel_util::kInverseNttNegacyclicHarveyLazy(
            temp_dest_Bsk.get(), dest_size, base_Bsk_size, coeff_power, base_Bsk_ntt_tables);
        ...
        {
            kernel_util::kMultiplyPolyScalarCoeffmod(
                temp_dest_q + i * coeff_count * base_q_size, 1, base_q_size, coeff_count, plain_modulus, base_q,
                temp_q_Bsk);
            kernel_util::kMultiplyPolyScalarCoeffmod(
                temp_dest_Bsk + i * coeff_count * base_Bsk_size, 1, base_Bsk_size, coeff_count, plain_modulus, base_Bsk,
    ...
    void EvaluatorCuda::ckksMultiply(CiphertextCuda &encrypted1, const CiphertextCuda &encrypted2) const
    {
        ...
        for (size_t i = 0; i < dest_size; i++)
        {
            ...
            kernel_util::kDyadicConvolutionCoeffmod(
                shifted_encrypted1_iter, shifted_reversed_encrypted2_iter, steps, coeff_modulus_size, coeff_count,
                coeff_modulus, temp + i * coeff_count * coeff_modulus_size);
        }

        kernel_util::kSetPolyArray(temp, dest_size, coeff_modulus_size, coeff_count, encrypted1.data());
    ...
    void EvaluatorCuda::bgvMultiply(CiphertextCuda &encrypted1, const CiphertextCuda &encrypted2) const
    {
        ...
        kernel_util::kNttNegacyclicHarvey(
            encrypted1.data(), encrypted1_size, coeff_modulus_size, coeff_power, ntt_table);
        ...
        else
        {
            ...
            kernel_util::kNttNegacyclicHarvey(
                encrypted2_cpy.data(), encrypted2_size, coeff_modulus_size, coeff_power, ntt_table);
        ...
        kernel_util::kInverseNttNegacyclicHarvey(
            encrypted1.data(), encrypted1.size(), coeff_modulus_size, coeff_power, ntt_table);
    ...
    void EvaluatorCuda::bfvSquare(CiphertextCuda &encrypted) const
    {
        ...
        {
            kernel_util::kSetPolyArray(
                encrypted.data(i), 1, base_q_size, coeff_count, encrypted_q.get() + i * coeff_count * base_q_size);
            kernel_util::kNttNegacyclicHarveyLazy(
                encrypted_q.get() + i * coeff_count * base_q_size, 1, base_q_size, coeff_power, base_q_ntt_tables);
            rns_tool->fastbconvmTilde(encrypted.data(i), temp.get());
            rns_tool->smMrq(temp.get(), encrypted_Bsk.get() + i * coeff_count * base_Bsk_size);
            kernel_util::kNttNegacyclicHarveyLazy(
                encrypted_Bsk.get() + i * coeff_count * base_Bsk_size, 1, base_Bsk_size, coeff_power,
                base_Bsk_ntt_tables);
        }
    ...
    void EvaluatorCuda::relinearizeInternal(
        CiphertextCuda &encrypted, const RelinKeysCuda &relin_keys, std::size_t destination_size) const
    {
        encrypted.seed() = 0;
        ...
        if (destination_size < 2 || destination_size > encrypted_size)
            throw invalid_argument("destination_size must be at least 2 and less than or equal to current "
                                   "count");
    ...
    void EvaluatorCuda::modSwitchScaleToNext(const CiphertextCuda &encrypted, CiphertextCuda &destination) const
    {
        destination.seed() = 0;
        ...
        {
            kernel_util::kSetPolyArray(
                encrypted_copy.data(i), 1, next_coeff_modulus_size, coeff_count, destination.data(i));
        }
        ...
        else if (next_parms.scheme() == SchemeType::bgv)
        {
            ...
            destination.correctionFactor() =
                multiplyUintMod(encrypted.correctionFactor(), rns_tool->invqLastModt(), next_parms.plainModulus());
        }
    }

    void EvaluatorCuda::modSwitchDropToNext(const CiphertextCuda &encrypted, CiphertextCuda &destination) const
    {
        destination.seed() = 0;
        // Assuming at this point encrypted is already validated.
        auto context_data_ptr = context_.getContextData(encrypted.parmsID());
        ...
        else
        {
            ...
            {
                kernel_util::kSetPolyArray(
                    encrypted.data(i), 1, next_coeff_modulus_size, coeff_count, destination.data(i));
            }
        }
    ...
    void EvaluatorCuda::modSwitchToNext(const CiphertextCuda &encrypted, CiphertextCuda &destination) const
    {
        destination.seed() = 0;
    ...
    __global__ void gSwitchKeyInplaceUtilA(
        uint64_t *t_poly_lazy, size_t coeff_count, size_t key_component_count, const uint64_t *key_vector_j,
        size_t key_poly_coeff_size, const uint64_t *t_operand, size_t key_index, const Modulus *key_modulus)
    {
    ...
    __global__ void gSwitchKeyInplaceUtilC(
        const uint64_t *t_poly_lazy, size_t coeff_count, size_t key_component_count, size_t rns_modulus_size,
    ...
    __global__ void gSwitchKeyInplaceUtilD(
        const uint64_t *t_poly_lazy, size_t coeff_count, size_t key_component_count, size_t rns_modulus_size,
        uint64_t *t_poly_prod_iter, size_t key_index, const Modulus *key_modulus)
    {
        GET_INDEX_COND_RETURN(coeff_count);
        FOR_N(k, key_component_count)
        {
    ...
    __global__ void gSwitchKeyInplaceUtilE(
        const uint64_t *t_last, uint64_t *t_poly_prod_i, size_t coeff_count, const Modulus *plain_modulus,
        const Modulus *key_modulus, size_t decomp_modulus_size, size_t rns_modulus_size, uint64_t qk_inv_qp,
        uint64_t qk, const MultiplyUIntModOperand *modswitch_factors, uint64_t *encrypted_i)
    {
        GET_INDEX_COND_RETURN(coeff_count);
        ...
        FOR_N(j, decomp_modulus_size)
        {
            ...
            encrypted_i[j * coeff_count + gindex] =
                kernel_util::dAddUintMod(target, encrypted_i[j * coeff_count + gindex], key_modulus[j]);
        }
    }

    __global__ void gSwitchKeyInplaceUtilF(
        uint64_t *t_last, size_t coeff_count, const Modulus *qk, const Modulus *key_modulus, uint64_t qk_half,
    ...
    __global__ void gSwitchKeyInplaceUtilG(
        uint64_t *t_poly_prod_i, const uint64_t *t_ntt, size_t coeff_count, uint64_t *encrypted_i, bool is_ckks,
        size_t decomp_modulus_size, const Modulus *key_modulus, const MultiplyUIntModOperand *modswitch_factors)
    {
        GET_INDEX_COND_RETURN(coeff_count);
        FOR_N(j, decomp_modulus_size)
        {
            ...
            encrypted_i[j * coeff_count + gindex] =
                kernel_util::dAddUintMod(encrypted_i[j * coeff_count + gindex], dest, key_modulus[j]);
        }
    }

    void EvaluatorCuda::switchKeyInplace(
        CiphertextCuda &encrypted, ConstDevicePointer<uint64_t> target_iter, const KSwitchKeysCuda &kswitch_keys,
        size_t kswitch_keys_index) const
    {
        ...
        for (size_t i = 0; i < rns_modulus_size; i++)
        {
            ...
            for (size_t j = 0; j < decomp_modulus_size; j++)
            {
                ...
                else
                {
                    ...
                    {
                        // printf("branch b - ");
                        kernel_util::kModuloPolyCoeffs(
                            t_target.get() + j * coeff_count, 1, 1, coeff_count, key_modulus + key_index, t_ntt.get());
                    }
                ...
                if (!lazy_reduction_counter)
                {
                    ...
                    (t_poly_lazy.get(), coeff_count, key_component_count, key_vector[j].data().data().get(),
                     key_vector_poly_coeff_size, t_operand.get(), key_index, key_modulus.get());
                }
                else
                {
            ...
            if (lazy_reduction_counter == lazy_reduction_summand_bound)
            {
                ...
                (t_poly_lazy.get(), coeff_count, key_component_count, rns_modulus_size, t_poly_prod_iter);
            }
            else
            {
                KERNEL_CALL(gSwitchKeyInplaceUtilD, coeff_count)
                (t_poly_lazy.get(), coeff_count, key_component_count, rns_modulus_size, t_poly_prod_iter, key_index,
                 key_modulus.get());
            }
        }
        ...
        for (size_t i = 0; i < key_component_count; i++)
        {
            if (scheme == SchemeType::bgv)
            {
                ...
                kernel_util::kInverseNttNegacyclicHarvey(
                    t_poly_prod + i * coeff_count * rns_modulus_size, 1, decomp_modulus_size, coeff_power,
                    key_ntt_tables);

                KERNEL_CALL(gSwitchKeyInplaceUtilE, coeff_count)
                (t_last.get(), t_poly_prod.get() + i * coeff_count * rns_modulus_size, coeff_count,
                 plain_modulus_cuda.get(), key_modulus.get(), decomp_modulus_size, rns_modulus_size, qk_inv_qp, qk,
            ...
            else
            {
                ...
                (t_last.get(), coeff_count, key_modulus.get() + (key_modulus_size - 1), key_modulus.get(),
                 key_modulus_cpu[key_modulus_size - 1].value() >> 1, decomp_modulus_size, t_ntt.get());
                ...
                gSwitchKeyInplaceUtilG<<<block_count, 256>>>(
                    t_poly_prod.get() + i * coeff_count * rns_modulus_size, t_ntt.get(), coeff_count,
                    encrypted.data(i).get(), scheme == SchemeType::ckks, decomp_modulus_size, key_modulus.get(),
                    modswitch_factors);
            }
    ...
    void EvaluatorCuda::applyKeySwitchingInplace(CiphertextCuda &encrypted, const KSwitchKeysCuda &kswitch_keys) const
    {
        ...
        kernel_util::kSetPolyArray(
            encrypted.data(1), 1, encrypted.coeffModulusSize(), encrypted.polyModulusDegree(), target.get());
        kernel_util::kSetZeroPolyArray(
            1, encrypted.coeffModulusSize(), encrypted.polyModulusDegree(), encrypted.data(1));
    ...
    void EvaluatorCuda::multiplyMany(
        const std::vector<CiphertextCuda> &encrypteds, const RelinKeysCuda &relin_keys,
    ...
    void EvaluatorCuda::addPlainInplace(CiphertextCuda &encrypted, const PlaintextCuda &plain) const
    {
        ...
        case SchemeType::ckks:
        {
            ...
            kernel_util::kAddPolyCoeffmod(
                encrypted_iter, plain_iter, 1, coeff_modulus_size, coeff_count, coeff_modulus, encrypted_iter);
            break;
        }
        ...
        case SchemeType::bgv:
        {
            ...
            kernel_util::kMultiplyPolyScalarCoeffmod(
                plain.data(), 1, 1, plain.coeffCount(), encrypted.correctionFactor(), parms.plainModulusCuda(),
                plain_copy.data());
    ...
    __global__ void gMultiplyPlainNormalUtilA(
        const uint64_t *plain_data, size_t plain_coeff_count, const uint64_t *plain_upper_half_increment,
        uint64_t plain_upper_half_threshold, size_t coeff_modulus_size, uint64_t *temp)
    {
        GET_INDEX_COND_RETURN(plain_coeff_count);
        uint64_t plain_value = plain_data[gindex];
        if (plain_value >= plain_upper_half_threshold)
        {
            kernel_util::dAddUint(
                plain_upper_half_increment, coeff_modulus_size, plain_value, temp + coeff_modulus_size * gindex);
        }
        else
        {
    ...
    __global__ void gMultiplyPlainNormalUtilB(
        ...
        const uint64_t *plain_upper_half_increment, uint64_t plain_upper_half_threshold, size_t coeff_modulus_size,
        uint64_t *temp)
    {
        GET_INDEX_COND_RETURN(plain_coeff_count);
        uint64_t plain_value = plain_data[gindex];
        ...
        {
            temp[i * coeff_count + gindex] = plain_value >= plain_upper_half_threshold
    ...
    void EvaluatorCuda::multiplyPlainNormal(CiphertextCuda &encrypted, const PlaintextCuda &plain) const
    {
        ...
        if (!context_data.qualifiers().using_fast_plain_lift)
        {
            ...
            (plain.data(), plain_coeff_count, plain_upper_half_increment.get(), plain_upper_half_threshold,
        ...
        else
        {
            ...
            (plain.data(), plain_coeff_count, coeff_count, plain_upper_half_increment.get(), plain_upper_half_threshold,
             coeff_modulus_size, temp.get());
        }
        ...
        for (size_t i = 0; i < encrypted_size; i++)
        {
            ...
            kernel_util::kDyadicProductCoeffmod(
                target_ptr, temp, 1, coeff_modulus_size, coeff_count, coeff_modulus, target_ptr);
    ...
    void EvaluatorCuda::multiplyPlainNtt(CiphertextCuda &encrypted_ntt, const PlaintextCuda &plain_ntt) const
    {
        ...
        {
            kernel_util::kDyadicProductCoeffmod(
                encrypted_ntt.data(i), plain_ntt_iter, 1, coeff_modulus_size, coeff_count, coeff_modulus,
    ...
    __global__ void gTransformToNttInplace(
        uint64_t *plain, size_t plain_coeff_count, size_t coeff_count, const uint64_t *plain_upper_half_increment,
    ...
    void EvaluatorCuda::transformToNttInplace(CiphertextCuda &encrypted) const
    {
        ...
        kernel_util::kNttNegacyclicHarvey(
            encrypted.data(), encrypted_size, coeff_modulus_size, getPowerOfTwo(coeff_count), ntt_tables);
    ...
    void EvaluatorCuda::transformFromNttInplace(CiphertextCuda &encrypted_ntt) const
    {
        ...
        kernel_util::kInverseNttNegacyclicHarvey(
            encrypted_ntt.data(), encrypted_ntt_size, coeff_modulus_size, getPowerOfTwo(coeff_count), ntt_tables);
        ...
        CiphertextCuda &encrypted, uint32_t galois_elt, const GaloisKeysCuda &galois_keys) const
    {
        ...
        if (parms.scheme() == SchemeType::bfv || parms.scheme() == SchemeType::bgv)
        {
            ...
            galois_tool->applyGalois(
                encrypted.data(0), 1, coeff_modulus_size, galois_elt, coeff_modulus.asPointer(), temp);
            ...
            galois_tool->applyGalois(
                encrypted.data(1), 1, coeff_modulus_size, galois_elt, coeff_modulus.asPointer(), temp);
        }
    ...
    void EvaluatorCuda::rotateInternal(CiphertextCuda &encrypted, int steps, const GaloisKeysCuda &galois_keys) const
    {
        encrypted.seed() = 0;
        auto context_data_ptr = context_.getContextData(encrypted.parmsID());
        if (!context_data_ptr)
    ...
    __global__ void assembleLWECopyC0(
        const uint64_t *c0, uint64_t *data, size_t offset, size_t coeff_count, size_t coeff_modulus_size)
    {
    ...
    CiphertextCuda EvaluatorCuda::assembleLWE(const LWECiphertextCuda &encrypted, size_t term) const
    {
        ...
        kernel_util::kNegacyclicShiftPolyCoeffmod(
            encrypted.c1().get(), 1, coeff_modulus_size, poly_modulus_degree, term, coeff_modulus.asPointer(),
            data.get() + poly_len);
        // copy c0
        KERNEL_CALL(assembleLWECopyC0, coeff_modulus_size)
        (encrypted.c0().get(), data.get(), term, poly_modulus_degree, coeff_modulus_size);
        return CiphertextCuda::fromMembers(
            encrypted.parmsID(), 2, coeff_modulus_size, poly_modulus_degree, false, encrypted.scale(),
            encrypted.correctionFactor(), util::DeviceDynamicArray(std::move(data)));
    }

    __global__ void extractLWECopyC0(
        const uint64_t *data, size_t offset, uint64_t *c0, size_t coeff_count, size_t coeff_modulus_size)
    {
        GET_INDEX_COND_RETURN(coeff_modulus_size);
        if (gindex < coeff_modulus_size)
        {
    ...
    LWECiphertextCuda EvaluatorCuda::extractLWE(const CiphertextCuda &encrypted, size_t term) const
    {
        ...
        else
        {
            ...
            kernel_util::kNegacyclicShiftPolyCoeffmod(
                encrypted.data(1), 1, coeff_modulus_size, poly_modulus_degree, shift, coeff_modulus, c1);
            // gather c0
            ...
            return LWECiphertextCuda::fromMembers(
                encrypted.parmsID(), poly_modulus_degree, coeff_modulus_size, encrypted.scale(),
                encrypted.correctionFactor(), std::move(c1), std::move(c0));
        }
    }

    void EvaluatorCuda::fieldTraceInplace(
        CiphertextCuda &encrypted, const GaloisKeysCuda &automorphism_keys, size_t logn) const
    {
    ...
    void EvaluatorCuda::divideByPolyModulusDegreeInplace(CiphertextCuda &encrypted, uint64_t mul) const
    {
        ...
        kernel_util::kMultiplyInvPolyDegreeCoeffmod(
            encrypted.data(), size, modulus.size(), context_data->parms().polyModulusDegree(), ntt_tables, modulus,
            encrypted.data());
        if (mul > 1)
        {
            kernel_util::kMultiplyPolyScalarCoeffmod(
                encrypted.data(), size, modulus.size(), context_data->parms().polyModulusDegree(), mul, modulus,
                encrypted.data());
        }
    }

    CiphertextCuda EvaluatorCuda::packLWECiphertexts(
        const std::vector<LWECiphertextCuda> &lwes, GaloisKeysCuda &automorphism_keys) const
    {
        ...
        for (size_t layer = 0; layer < l; layer++)
        {
            ...
            while (offset < (1 << l))
            {
                ...
                kernel_util::kNegacyclicShiftPolyCoeffmod(
                    odd.data(), odd.size(), coeff_modulus_size, poly_modulus_degree, shift, coeff_modulus, temp.data());
                // add
                sub(even, temp, odd);
    ...
    void EvaluatorCuda::negacyclicShift(
        const CiphertextCuda &encrypted, size_t shift, CiphertextCuda &destination) const
    {
        ...
        auto coeff_modulus = context_data->parms().coeffModulus();
        kernel_util::kNegacyclicShiftPolyCoeffmod(
            encrypted.data(), encrypted.size(), coeff_modulus_size, poly_modulus_degree, shift, coeff_modulus,
            destination.data());
    }

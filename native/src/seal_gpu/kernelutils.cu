...
namespace seal_gpu
{
    ...
    namespace kernel_util
    {
        ...
        __global__ void gAddPolyCoeffmod(
            const uint64_t *operand1, const uint64_t *operand2, POLY_ARRAY_ARGUMENTS, const Modulus *modulus,
            uint64_t *result)
        {
        ...
        void kAddPolyCoeffmod(
            CPointer operand1, CPointer operand2, POLY_ARRAY_ARGUMENTS, MPointer modulus, Pointer result)
        {
            KERNEL_CALL(gAddPolyCoeffmod, poly_modulus_degree)
            (operand1.get(), operand2.get(), POLY_ARRAY_ARGCALL, modulus.get(), result.get());
        }

        __device__ inline uint64_t dDyadicSingle(
            uint64_t o1, uint64_t o2, uint64_t modulus_value, uint64_t const_ratio_0, uint64_t const_ratio_1)
        {
        ...
        __global__ void gDyadicConvolutionCoeffmod(
            const uint64_t *operand1, const uint64_t *operand2_reversed, POLY_ARRAY_ARGUMENTS, const Modulus *moduli,
            uint64_t *single_poly_result_accumulator)
        {
            ...
            FOR_N(rns_index, coeff_modulus_size)
            {
                ...
                {
                    const uint64_t *o1 =
                        operand1 + (poly_index * coeff_modulus_size + rns_index) * poly_modulus_degree + gindex;
                    ...
                    uint64_t *res = single_poly_result_accumulator + rns_index * poly_modulus_degree + gindex;
        ...
        void kDyadicConvolutionCoeffmod(
            CPointer operand1, CPointer operand2_reversed, POLY_ARRAY_ARGUMENTS, MPointer moduli,
        ...
        __global__ void gDyadicProductCoeffmod(
            const uint64_t *operand1, const uint64_t *operand2, POLY_ARRAY_ARGUMENTS, const Modulus *moduli,
            uint64_t *output)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
        ...
        void kDyadicProductCoeffmod(
            CPointer operand1, CPointer operand2, POLY_ARRAY_ARGUMENTS, MPointer moduli, Pointer output)
        {
            KERNEL_CALL(gDyadicProductCoeffmod, poly_modulus_degree)
            (operand1.get(), operand2.get(), POLY_ARRAY_ARGCALL, moduli.get(), output.get());
        }

        __global__ void gDyadicSquareCoeffmod(
            const uint64_t *operand, size_t coeff_modulus_size, size_t poly_modulus_degree, const Modulus *moduli,
            uint64_t *output)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            ...
            FOR_N(rns_index, coeff_modulus_size)
            {
                ...
                output[2 * d + id] = dDyadicSingle(
                    operand[1 * d + id], operand[1 * d + id], modulus_value, const_ratio_0, const_ratio_1);
                uint64_t cross = dDyadicSingle(
                    operand[0 * d + id], operand[1 * d + id], modulus_value, const_ratio_0, const_ratio_1);
                cross += cross;
                output[1 * d + id] = cross >= modulus_value ? cross - modulus_value : cross;
                output[0 * d + id] = dDyadicSingle(
                    operand[0 * d + id], operand[0 * d + id], modulus_value, const_ratio_0, const_ratio_1);
            }
        }

        void kDyadicSquareCoeffmod(
            CPointer operand, size_t coeff_modulus_size, size_t poly_modulus_degree, MPointer moduli, Pointer output)
        {
            KERNEL_CALL(gDyadicSquareCoeffmod, poly_modulus_degree)
            (operand.get(), coeff_modulus_size, poly_modulus_degree, moduli.get(), output.get());
        }

        __global__ void gModBoundedUsingNttTables(
            uint64_t *operand, POLY_ARRAY_ARGUMENTS, const NTTTablesCuda *ntt_tables)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
                ...
                FOR_N(poly_index, poly_size)
                {
                    ...
                    if (operand[id] >= modulus_value)
                        operand[id] -= modulus_value;
                }
            }
        }

        void kModBoundedUsingNttTables(
            Pointer operand, POLY_ARRAY_ARGUMENTS, ConstDevicePointer<NTTTablesCuda> ntt_tables)
        {
        ...
        __global__ void gModuloPolyCoeffs(
            const uint64_t *operand, POLY_ARRAY_ARGUMENTS, const Modulus *moduli, uint64_t *result)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
        ...
        void kModuloPolyCoeffs(CPointer operand, POLY_ARRAY_ARGUMENTS, MPointer moduli, Pointer result)
        {
        ...
        __global__ void gMultiplyInvDegreeNttTables(
            uint64_t *poly_array, POLY_ARRAY_ARGUMENTS, const NTTTablesCuda *ntt_tables)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
        ...
        __global__ void gMultiplyPolyScalarCoeffmod(
            const uint64_t *poly_array, POLY_ARRAY_ARGUMENTS, const MultiplyUIntModOperand *reduced_scalar,
        ...
        void kMultiplyPolyScalarCoeffmod(
            CPointer poly_array, POLY_ARRAY_ARGUMENTS, uint64_t scalar, MPointer modulus, Pointer result)
        {
            ...
            (poly_array.get(), POLY_ARRAY_ARGCALL, reduced_scalar.get(), modulus.get(), result.get());
        }

        __global__ void gNegatePolyCoeffmod(
            const uint64_t *poly_array, POLY_ARRAY_ARGUMENTS, const Modulus *modulus, uint64_t *result)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
        ...
        void kNegatePolyCoeffmod(CPointer poly_array, POLY_ARRAY_ARGUMENTS, MPointer modulus, Pointer result)
        {
        ...
        void kNttTransferToRev(
            Pointer operand, size_t poly_size, size_t coeff_modulus_size, size_t poly_modulus_degree_power,
            ConstDevicePointer<NTTTablesCuda> ntt_tables, bool use_inv_root_powers)
        {
            std::size_t m = 1;
            std::size_t layer = 0;
            ...
            for (; m <= (n >> 1); m <<= 1)
            {
                kNttTransferToRevLayered(
                    layer, operand, poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables,
                    use_inv_root_powers);
                layer++;
            }
        }
        ...
        __global__ void gNttTransferToRevLayered(
            size_t layer, uint64_t *operand, size_t poly_size, size_t coeff_modulus_size,
            size_t poly_modulus_degree_power, const NTTTablesCuda *ntt_tables, bool use_inv_root_powers)
        {
            ...
            FOR_N(rns_index, coeff_modulus_size)
            {
                ...
                MultiplyUIntModOperand r = use_inv_root_powers ? (ntt_tables[rns_index].getFromInvRootPowers()[rid])
                                                               : (ntt_tables[rns_index].getFromRootPowers()[rid]);
                FOR_N(poly_index, poly_size)
                {
                    uint64_t *x = operand +
                                  ((poly_index * coeff_modulus_size + rns_index) << poly_modulus_degree_power) +
                                  coeff_index;
                    uint64_t *y = x + gap;
        ...
        void kNttTransferToRevLayered(
            size_t layer, Pointer operand, size_t poly_size, size_t coeff_modulus_size,
            size_t poly_modulus_degree_power, ConstDevicePointer<NTTTablesCuda> ntt_tables, bool use_inv_root_powers)
        {
            std::size_t n = size_t(1) << poly_modulus_degree_power;
            KERNEL_CALL(gNttTransferToRevLayered, n)
            (layer, operand.get(), poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables.get(),
             use_inv_root_powers);
        }
        ...
        __global__ void gSetMultiplyUIntModOperand(
            uint64_t scalar, const Modulus *moduli, size_t n, MultiplyUIntModOperand *result)
        {
        ...
        __global__ void gNegacyclicShiftPolyCoeffmod(
            const uint64_t *poly, POLY_ARRAY_ARGUMENTS, size_t shift, const Modulus *modulus, uint64_t *result)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
        ...
        void kNegacyclicShiftPolyCoeffmod(
            CPointer poly, POLY_ARRAY_ARGUMENTS, size_t shift, MPointer modulus, Pointer result)
        {
        ...
        __global__ void gMultiplyInvPolyDegreeCoeffmod(
            const uint64_t *poly_array, POLY_ARRAY_ARGUMENTS, const NTTTablesCuda *ntt_tables, const Modulus *modulus,
            uint64_t *result)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
                FOR_N(poly_index, poly_size)
                {
                    size_t id = (poly_index * coeff_modulus_size + rns_index) * poly_modulus_degree + gindex;
                    result[id] =
                        dMultiplyUintMod(poly_array[id], ntt_tables[rns_index].invDegreeModulo(), modulus[rns_index]);
                }
            }
        }

        void kMultiplyInvPolyDegreeCoeffmod(
            CPointer poly_array, POLY_ARRAY_ARGUMENTS, ConstDevicePointer<NTTTablesCuda> ntt_tables, MPointer modulus,
            Pointer result)
        {
            assert(coeff_modulus_size <= 256);
            KERNEL_CALL(gMultiplyInvPolyDegreeCoeffmod, poly_modulus_degree)
            (poly_array.get(), POLY_ARRAY_ARGCALL, ntt_tables.get(), modulus.get(), result.get());
        }

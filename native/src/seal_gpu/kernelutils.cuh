...
namespace seal_gpu
{
    ...
    public:
        ...
        const __device__ inline static uint64_t *getModulusConstRatio(const Modulus &m)
        {
    ...
    namespace kernel_util
    {
        ...
        __device__ inline unsigned char dAddUint64(
        ...
        {
            *hw64 =
                static_cast<uint64_t>(((static_cast<uint128_t>(operand1) * static_cast<uint128_t>(operand2)) >> 64));
        }
        ...
        __device__ inline unsigned char dSubUint64(
            uint64_t operand1, uint64_t operand2, unsigned char borrow, uint64_t *result)
        {
        ...
        __device__ inline unsigned char dAddUint(
            const uint64_t *operand1, const uint64_t *operand2, std::size_t uint64Count, uint64_t *result)
        {
        ...
        __device__ inline int dCompareUint(const uint64_t *operand1, const uint64_t *operand2, std::size_t uint64_count)
        {
            int result = 0;
        ...
        __device__ inline uint64_t dSubUintMod(std::uint64_t operand1, std::uint64_t operand2, const Modulus &modulus)
        {
        ...
        __device__ inline uint64_t dMultiplyScalarMod(uint64_t operand, std::uint64_t scalar, const Modulus &modulus)
        {
            ...
            MultiplyUIntModOperand temp_scalar = dSetMultiplyModOperand(dBarrettReduce64(scalar, modulus), modulus);
            return dMultiplyUintMod(operand, temp_scalar, modulus);
        }

        __device__ inline unsigned char dAddUint(
            const uint64_t *operand1, std::size_t uint64Count, uint64_t operand2, uint64_t *result)
        {
            // Unroll first iteration of loop. We assume uint64_count > 0.
            unsigned char carry = dAddUint64(*operand1++, operand2, result++);
        ...
        inline void kDyadicSquareCoeffmod(
            Pointer operand, size_t coeff_modulus_size, size_t poly_modulus_degree, MPointer moduli)
        {
            kDyadicSquareCoeffmod(operand, coeff_modulus_size, poly_modulus_degree, moduli, operand);
        }
        ...
        {
            kNttTransferToRev(operand, poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables, false);
        }
        ...
        inline void kNttNegacyclicHarvey(
            ...
            ConstDevicePointer<NTTTablesCuda> ntt_tables)
        {
            kNttNegacyclicHarveyLazy(operand, poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables);
            kModBoundedUsingNttTables(
                operand, poly_size, coeff_modulus_size, 1 << poly_modulus_degree_power, ntt_tables);
        }
        ...
        inline void kInverseNttNegacyclicHarveyLazy(
            ...
            ConstDevicePointer<NTTTablesCuda> ntt_tables)
        {
            kNttTransferFromRev(operand, poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables, true);
        }
            ...
            ConstDevicePointer<NTTTablesCuda> ntt_tables)
        {
            kInverseNttNegacyclicHarveyLazy(
                operand, poly_size, coeff_modulus_size, poly_modulus_degree_power, ntt_tables);
            kModBoundedUsingNttTables(
                operand, poly_size, coeff_modulus_size, 1 << poly_modulus_degree_power, ntt_tables);
        }
        ...
        inline void kSetPolyArray(CPointer poly, POLY_ARRAY_ARGUMENTS, Pointer result)
        {
            KernelProvider::copyOnDevice(
                result.get(), poly.get(), poly_size * coeff_modulus_size * poly_modulus_degree);
        }

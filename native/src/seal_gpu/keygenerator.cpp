...
namespace seal_gpu
{
    ...
    RelinKeys KeyGenerator::createRelinKeys(size_t count)
    {
        ...
        generateKswitchKeys(
            secret_key + coeff_count * coeff_modulus_size, count, static_cast<KSwitchKeys &>(relin_keys));
    ...
    void KeyGenerator::computeSecretKeyArray(const SEALContext::ContextData &context_data, size_t max_power)
    {
        ...
        for (size_t i = 0; i < new_size - old_size; i++)
        {
            dyadicProductCoeffmod(
                secret_key_power + i * poly_diff, secret_key, coeff_modulus_size, coeff_count, &coeff_modulus[0],
    ...
    void KeyGenerator::generateOneKswitchKey(ConstHostPointer<uint64_t> new_key, vector<PublicKey> &destination)
    {
        ...
        for (size_t i = 0; i < decomp_mod_count; i++)
        {
            ...
            encryptZeroSymmetric(secret_key_, context_, keyContextData.parmsID(), true, destination[i].data());
            uint64_t factor = barrettReduce64(key_modulus.back().value(), key_modulus[i]);
            multiplyPolyScalarCoeffmod(
                new_key + i * coeff_count, coeff_count, factor, key_modulus[i], temp.asPointer());

...
namespace seal_gpu
{
    ...
    public:
        ...
        explicit PlaintextCuda(std::size_t coeff_count) : coeff_count_(coeff_count), data_(coeff_count_)
        {}
        ...
        PlaintextCuda(const Plaintext &copy)
            : parms_id_(copy.parmsID()), coeff_count_(copy.coeffCount()), scale_(copy.scale()), data_(copy.dynArray())
        {}
        ...
        PlaintextCuda(const std::string &hex_poly) : data_()
        {
            operator=(hex_poly);
        }
        ...
        inline void setZero(std::size_t start_coeff, std::size_t length)
        {
            ...
            {
                throw std::out_of_range("length must be non-negative and start_coeff + length - 1 must be "

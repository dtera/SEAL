...
namespace seal_gpu
{
    namespace util
    {
        ...
        public:
            ...
            DevicePointer<T> asPointer()
            {
                return DevicePointer<T>(data);
            }
            ...
            T back() const
            {
                T ret;
                if (data)
                    KernelProvider::retrieve(&ret, data + len - 1, 1);
                return ret;
            }
        ...
        public:
            ...
            DeviceDynamicArray(DeviceArray<T> &&move, size_t size) : internal(std::move(move)), size_(size)
            {}
            ...
            DeviceDynamicArray(const HostDynamicArray<T> &h) : size_(h.size()), internal(h.internal)
            {}

...
namespace seal_gpu
{
    namespace util
    {
        ...
        public:
            ...
            void applyGalois(
                ConstHostPointer<uint64_t> operand, std::uint32_t galois_elt, const Modulus &modulus,
            ...
            void applyGalois(
                ConstHostPointer<uint64_t> operand, std::size_t poly_size, std::size_t coeff_modulus_size,
                std::uint32_t galois_elt, const Modulus *modulus, HostPointer<uint64_t> result) const
            {
            ...
            void applyGaloisNtt(
                ConstHostPointer<uint64_t> operand, std::uint32_t galois_elt, HostPointer<uint64_t> result) const;
            ...
            void applyGaloisNtt(
                ConstHostPointer<uint64_t> operand, std::size_t poly_size, std::size_t coeff_modulus_size,
                std::uint32_t galois_elt, HostPointer<uint64_t> result) const
            {
                size_t d = coeff_modulus_size * coeff_count_;
                for (size_t i = 0; i < poly_size; i++)
                {

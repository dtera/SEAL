...
namespace seal_gpu
{
    namespace util
    {
        ...
        __global__ void gApplyGalois(
            const uint64_t *operand, size_t poly_size, size_t coeff_modulus_size, size_t coeff_count,
            size_t coeff_power, uint32_t galois_elt, const Modulus *modulus, uint64_t *result)
        {
            ...
            FOR_N(rns_index, coeff_modulus_size)
            {
                ...
                FOR_N(poly_index, poly_size)
                {
                    uint64_t result_value =
                        operand[(poly_index * coeff_modulus_size + rns_index) * coeff_count + gindex];
        ...
        void GaloisToolCuda::applyGalois(
            ConstDevicePointer<uint64_t> operand, size_t poly_size, size_t coeff_modulus_size, uint32_t galois_elt,
            ConstDevicePointer<Modulus> modulus, DevicePointer<uint64_t> result) const
        {
            KERNEL_CALL(gApplyGalois, coeff_count_)
            (operand.get(), poly_size, coeff_modulus_size, coeff_count_, getPowerOfTwo(coeff_count_), galois_elt,
        ...
        void GaloisToolCuda::applyGaloisNtt(
            ConstDevicePointer<uint64_t> operand, size_t poly_size, size_t coeff_modulus_size, uint32_t galois_elt,

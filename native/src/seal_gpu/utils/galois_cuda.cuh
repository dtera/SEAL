...
namespace seal_gpu
{
    namespace util
    {
        ...
        public:
            ...
            void applyGalois(
                ConstDevicePointer<uint64_t> operand, std::size_t poly_size, std::size_t coeff_modulus_size,
                std::uint32_t galois_elt, ConstDevicePointer<Modulus> modulus, DevicePointer<uint64_t> result) const;

            void applyGaloisNtt(
                ConstDevicePointer<uint64_t> operand, std::size_t poly_size, std::size_t coeff_modulus_size,
                std::uint32_t galois_elt, DevicePointer<uint64_t> result) const;

...
namespace seal_gpu
{
    namespace util
    {
        ...
        void addPolyScalarCoeffmod(
            ConstHostPointer<uint64_t> poly, size_t coeff_count, uint64_t scalar, const Modulus &modulus,
            HostPointer<uint64_t> result)
        {
            for (std::size_t i = 0; i < coeff_count; i++)
            {
        ...
        void dyadicProductCoeffmod(
            ConstHostPointer<uint64_t> operand1, ConstHostPointer<uint64_t> operand2, size_t coeff_count,
            const Modulus &modulus, HostPointer<uint64_t> result)
        {
            const uint64_t modulus_value = modulus.value();
        ...
        void negacyclicShiftPolyCoeffmod(
            ConstHostPointer<uint64_t> poly, size_t coeff_count, size_t shift, const Modulus &modulus,
            HostPointer<uint64_t> result)
        {
            // Nothing to do
            if (shift == 0)

...
namespace seal_gpu
{
    namespace util
    {
            ...
            shared_ptr<UniformRandomGenerator> prng, const EncryptionParameters &parms, uint64_t *destination)
        {
            ...
            {
                throw logic_error("centered binomial distribution only supports standard deviation 3.2; "
                                  "use rounded "
                                  "Gaussian instead");
            }
            ...
            for (size_t j = 0; j < encrypted_size; j++)
            {
                ...
                if (type == SchemeType::bgv)
                {
                    ...
                    multiplyPolyScalarCoeffmod(
                        gaussian_iter.toConst(), coeff_modulus_size, coeff_count, plain_modulus.value(),
                ...
                addPolyCoeffmod(
                    gaussian_iter.toConst(), dst_iter.toConst(), coeff_modulus_size, coeff_count, &coeff_modulus[0],
                    dst_iter);
            }
        }

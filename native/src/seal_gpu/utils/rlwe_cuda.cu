    ...
    namespace util
    {
        ...
        namespace sampler
        {
            ...
            __global__ void gSamplePolyTenary(
                curandState *states, size_t coeff_modulus_size, size_t coeff_count, const Modulus *modulus,
            ...
            inline void kSamplePolyTenary(
                DevicePointer<curandState> states, size_t coeff_modulus_size, size_t coeff_count,
                ConstDevicePointer<Modulus> modulus, DevicePointer<uint64_t> destination)
            {
                ...
                uint64_t *destination)
            {
                ...
                while (true)
                {
                    ...
                    if (r < noise_max_deviation / standard_deviation)
                        break;
                }
                ...
                FOR_N(j, coeff_modulus_size)
                {
                    ...
                    if (ri >= 0)
                        ...
                        destination[gindex + j * coeff_count] = modulus_value - static_cast<uint64_t>(-ri);
                }
            }
                ...
                uint64_t *destination)
            {
                ...
                FOR_N(j, coeff_modulus_size)
                {
                    ...
                    if (r >= 0)
                        ...
                        destination[gindex + j * coeff_count] = modulus_value - static_cast<uint64_t>(-r);
                }
            }
                ...
                uint64_t *destination)
            {
                ...
                FOR_N(j, coeff_modulus_size)
                {
                    destination[gindex + j * coeff_count] =
                        curand_uniform_double(states + gindex) * DeviceHelper::getModulusValue(modulus[j]);
                }
            }
            ...
            CiphertextCuda &destination, DevicePointer<curandState> curandStates)
        {
            ...
            for (size_t j = 0; j < encrypted_size; j++)
            {
                kernel_util::kDyadicProductCoeffmod(
                    u, public_key.data().data(j), 1, coeff_modulus_size, coeff_count, coeff_modulus,
                    destination.data(j));
                if (!is_ntt_form)
                {
            ...
            for (size_t j = 0; j < encrypted_size; j++)
            {
                ...
                if (type == SchemeType::bgv)
                {
                    ...
                    kernel_util::kMultiplyPolyScalarCoeffmod(
                        gaussian_iter, 1, coeff_modulus_size, coeff_count, plain_modulus.value(), coeff_modulus,
                        gaussian_iter);
                }
                else
                {
                ...
                kernel_util::kAddPolyCoeffmod(
                    gaussian_iter, destination.data(j), 1, coeff_modulus_size, coeff_count, coeff_modulus,
                    destination.data(j));
            }
        }
            ...
            CiphertextCuda &destination, DevicePointer<curandState> curandStates)
        {
            ...
            kernel_util::kDyadicProductCoeffmod(
                secret_key.data().data(), c1, 1, coeff_modulus_size, coeff_count, coeff_modulus, c0);
            if (is_ntt_form)
            {
            ...
            {
                kernel_util::kMultiplyPolyScalarCoeffmod(
                    noise, 1, coeff_modulus_size, coeff_count, plain_modulus.value(), coeff_modulus, noise);
            }

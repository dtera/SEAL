    ...
    namespace util
    {
        ...
        void RNSBase::compose(uint64_t *value) const
        {
            ...
            if (size_ > 1)
            {
                ...
                {
                    uint64_t temp_prod =
                        multiplyUintMod(temp_value[i], inv_punctured_prod_mod_base_array_[i], base_[i]);
        ...
        void RNSBase::composeArray(uint64_t *value, size_t count) const
        {
            ...
            if (size_ > 1)
            {
                ...
                for (size_t i = 0; i < count; i++)
                {
                    ...
                    {
                        uint64_t temp_prod =
                            multiplyUintMod(temp_array[i * size_ + j], inv_punctured_prod_mod_base_array_[j], base_[j]);
        ...
        void BaseConverter::fastConvertArray(
            ConstHostPointer<uint64_t> in, HostPointer<uint64_t> out, size_t count) const
        {
                ...
                else
                {
                    ...
                    for (size_t j = 0; j < count; j++)
                    {
                        ...
                        temp[j * ibase_size + ibase_index] = multiplyUintMod(
                            in[i * count + j], ibase_.invPuncturedProdModBaseArray()[i], ibase_.base()[i]);
                    }
                }
            }
            ...
            for (size_t i = 0; i < obase_size; i++)
            {
                for (size_t j = 0; j < count; j++)
                {
                    ...
                    out[i * count + j] = dotProductMod(
                        temp.get() + j * ibase_size, base_change_matrix_[i].get(), ibase_size, obase_.base()[i]);
                }
            }
        }
        ...
        void BaseConverter::exactConvertArray(
            ConstHostPointer<uint64_t> in, HostPointer<uint64_t> out, size_t in_count) const
        {
            size_t ibase_size = ibase_.size();
            size_t obase_size = obase_.size();
                ...
                else
                {
                    ...
                    for (size_t j = 0; j < count; j++)
                    {
                        ...
                        temp[j * ibase_size + ibase_index] =
                            multiplyUintMod(in[i * count + j], ibase_.invPuncturedProdModBaseArray()[i], ibase_modulus);
                        double_t dividend = static_cast<double_t>(temp[j * ibase_size + ibase_index]);
                        v[j * ibase_size + ibase_index] = dividend / divisor;
                    }
                }
            ...
            for (size_t j = 0; j < count; j++)
            {
                ...
                // Compute the base conversion sum modulo obase element
                auto sum_mod_obase =
                    dotProductMod(temp.get() + j * ibase_size, base_change_matrix_first, ibase_size, p);
                // Minus v*[q]_{p} mod p
            ...
            for (size_t i = 0; i < obase_.size(); i++)
            {
                ...
                for (size_t j = 0; j < ibase_.size(); j++)
                {
                    ...
                    base_change_matrix_[i][j] =
                        moduloUint(ibase_.puncturedProdArray() + ibase_.size() * j, ibase_.size(), obase_.base()[i]);
                }
            }
        }

        RNSTool::RNSTool(size_t poly_modulus_degree, const RNSBase &coeff_modulus, const Modulus &plain_modulus)
        {
        ...
        void RNSTool::divideAndRoundqLastInplace(HostPointer<uint64_t> input) const
        {
            ...
            for (size_t i = 0; i < base_q_size - 1; i++)
            {
                ...
                multiplyPolyScalarCoeffmod(
                    input + i * coeff_count_, coeff_count_, inv_q_last_mod_q_[i], b, input + i * coeff_count_);
            }
        }

        void RNSTool::divideAndRoundqLastNttInplace(HostPointer<uint64_t> input, const NTTTables *rns_ntt_tables) const
        {
            size_t base_q_size = base_q_->size();
            auto last_input = input + (base_q_size - 1) * coeff_count_;
            ...
            */
            ...
            for (size_t i = 0; i < coeff_count_; i++)
            {
                ...
                alpha_sk[i] = multiplyUintMod(
                    temp[i] + (m_sk_.value() - input[base_B_size * coeff_count_ + i]), inv_prod_B_mod_m_sk_, m_sk_);
            }
            ...
            for (size_t i = 0; i < base_q_size; i++)
            {
                ...
                for (size_t j = 0; j < coeff_count_; j++)
                {
                    ...
                    {
                        dest = multiplyAddUintMod(negateUintMod(alpha_sk[j], m_sk_), prod_B_mod_q_elt, dest, b);
                    }
            ...
            for (size_t i = 0; i < base_Bsk_size; i++)
            {
                ...
                for (size_t j = 0; j < coeff_count_; j++)
                {
                    ...
                    destination[i * coeff_count_ + j] = multiplyUintMod(
                        multiplyAddUintMod(temp, prod_q_mod_Bsk_elt, input[i * coeff_count_ + j], b),
                ...
                for (size_t j = 0; j < coeff_count_; j++)
                {
                    ...
                    destination[i * coeff_count_ + j] = multiplyUintMod(
                        input[i * coeff_count_ + j] +
                            (base_Bsk_->base()[i].value() - destination[i * coeff_count_ + j]),
        ...
        void RNSTool::fastbconvmTilde(ConstHostPointer<uint64_t> input, HostPointer<uint64_t> destination) const
        {
            ...
            */
            ...
            multiplyPolyScalarCoeffmod(
                input, base_q_size, coeff_count_, m_tilde_.value(), base_q_->base(), temp.asPointer());
            ...
            base_q_to_m_tilde_conv_->fastConvertArray(
                temp.asPointer(), destination + base_Bsk_size * coeff_count_, coeff_count_);
        }
        ...
        void RNSTool::decryptScaleAndRound(ConstHostPointer<uint64_t> input, HostPointer<uint64_t> destination) const
        {
            ...
            for (size_t i = 0; i < base_q_size; i++)
            {
                ...
                // base_q_size, [&](auto I) {
                multiplyPolyScalarCoeffmod(
                    input + i * coeff_count_, coeff_count_, prod_t_gamma_mod_q_[i], base_q_->base()[i],
                    temp + coeff_count_ * i);
            }
            ...
            for (size_t i = 0; i < base_t_gamma_size; i++)
            {
                ...
                // [&](auto I) {
                multiplyPolyScalarCoeffmod(
                    temp_t_gamma + i * coeff_count_, coeff_count_, neg_inv_q_mod_t_gamma_[i], base_t_gamma_->base()[i],
            ...
            for (size_t i = 0; i < coeff_count_; i++)
            {
                ...
                if (temp_t_gamma[coeff_count_ + i] > gamma_div_2)
                {
                    ...
                    destination[i] = addUintMod(
                        temp_t_gamma[i], barrettReduce64(gamma_.value() - temp_t_gamma[coeff_count_ + i], t_), t_);
                }
                // No correction needed
                else
                {
                    destination[i] =
                        subUintMod(temp_t_gamma[i], barrettReduce64(temp_t_gamma[coeff_count_ + i], t_), t_);
                }
        ...
        void RNSTool::modTAndDivideqLastInplace(HostPointer<uint64_t> input) const
        {
            ...
            moduloPolyCoeffs(
                input + (modulus_size - 1) * coeff_count_, coeff_count_, plain_modulus, neg_c_last_mod_t.asPointer());
            ...
            if (inv_q_last_mod_t_ != 1)
            {
                ...
                multiplyPolyScalarCoeffmod(
                    neg_c_last_mod_t.asPointer(), coeff_count_, inv_q_last_mod_t_, plain_modulus,
            ...
            for (size_t i = 0; i < modulus_size - 1; i++)
            {
                ...
                moduloPolyCoeffs(
                    neg_c_last_mod_t.asPointer(), coeff_count_, curr_modulus[i], delta_mod_q_i.asPointer());

                // delta_mod_q_i *= q_last (mod q_i)
                multiplyPolyScalarCoeffmod(
                    delta_mod_q_i.asPointer(), coeff_count_, last_modulus_value, curr_modulus[i],
                ...
                for (size_t j = 0; j < coeff_count_; j++)
                {
                    ...
                    // coeff_count_, [&](auto J) {
                    input[i * coeff_count_ + j] +=
                        two_times_q_i - barrettReduce64(input[(modulus_size - 1) * coeff_count_ + j], curr_modulus[i]) -
                        delta_mod_q_i[j];
                }

                // c_i = c_i * inv_q_last_mod_q_i (mod q_i)
                multiplyPolyScalarCoeffmod(
                    input + i * coeff_count_, coeff_count_, inv_q_last_mod_q_[i], curr_modulus[i],

...
namespace seal_gpu
{
    ...
    namespace util
    {
        ...
        __global__ void gDecomposeArray(const uint64_t *values, POLY_ARGUMENTS, const Modulus *moduli, uint64_t *result)
        {
        ...
        __global__ void gDecomposeArrayKeepOrder(
            const uint64_t *values, POLY_ARGUMENTS, const Modulus *moduli, uint64_t *result)
        {
            GET_INDEX_COND_RETURN(poly_modulus_degree);
            FOR_N(rns_index, coeff_modulus_size)
            {
        ...
        __global__ void gComposeArray(
            const uint64_t *value, POLY_ARGUMENTS, const Modulus *moduli, const uint64_t *base_prod,
            const uint64_t *punctured_prod_array, const MultiplyUIntModOperand *inv_punctured_prod_mod_base_array,
            uint64_t *temp, uint64_t *result)
        {
            ...
            FOR_N(rns_index, coeff_modulus_size)
            {
                uint64_t temp_prod = kernel_util::dMultiplyUintMod(
                    value[rns_index * poly_modulus_degree + gindex], inv_punctured_prod_mod_base_array[rns_index],
                    moduli[rns_index]);
                kernel_util::dMultiplyUint(
                    punctured_prod_array + rns_index * coeff_modulus_size, coeff_modulus_size, temp_prod,
                    coeff_modulus_size, temp_target);
                kernel_util::dAddUintUintMod(
                    temp_target, result + gindex * coeff_modulus_size, base_prod, coeff_modulus_size,
        ...
        void RNSBaseCuda::composeArray(DevicePointer<uint64_t> values, size_t coeff_count) const
        {
            ...
            (copy.get(), size_, coeff_count, base_.get(), base_prod_.get(), punctured_prod_array_.get(),
        ...
        __global__ void gFastConvertArrayStepA(
            ...
            const MultiplyUIntModOperand *inv_punctured_prod_mod_base_array, uint64_t *out)
        {
        ...
        __global__ void gFastConvertArrayStepB(
            const uint64_t *in, size_t ibase_size, size_t obase_size, size_t count, const Modulus *moduli,
            const uint64_t *base_change_matrix_, uint64_t *out)
        {
            GET_INDEX_COND_RETURN(count);
            FOR_N(i, obase_size)
            {
                out[i * count + gindex] = kernel_util::dDotProductMod(
                    in + gindex * ibase_size, base_change_matrix_ + i * ibase_size, ibase_size, moduli[i]);
            }
        }

        void BaseConverterCuda::fastConvertArray(
            ConstDevicePointer<uint64_t> in, DevicePointer<uint64_t> out, size_t count) const
        {
            ...
            (in.get(), ibase_size, count, ibase_.base().get(), ibase_.invPuncturedProdModBaseArray().get(), temp.get());
            gFastConvertArrayStepB<<<block_count, 256>>>(
                temp.get(), ibase_size, obase_size, count, obase_.base().get(), base_change_matrix_.get(), out.get());
        }

        __global__ void gExactConvertArray(
            const uint64_t *in, size_t ibase_size, size_t count, const Modulus *ibase,
            const MultiplyUIntModOperand *inv_punctured_prod_mod_base_array, const uint64_t *ibase_prod,
            const Modulus *obase, const uint64_t *base_change_matrix, uint64_t *temp, uint64_t *out)
        {
            GET_INDEX_COND_RETURN(count);
            double aggregated = 0;
            FOR_N(i, ibase_size)
            ...
            uint64_t sum_mod_obase =
                kernel_util::dDotProductMod(temp + gindex * ibase_size, base_change_matrix, ibase_size, p);
        ...
        void BaseConverterCuda::exactConvertArray(
            ConstDevicePointer<uint64_t> in, DevicePointer<uint64_t> out, size_t count) const
        {
            size_t ibase_size = ibase_.size();
            size_t obase_size = obase_.size();
            ...
            (in.get(), ibase_size, count, ibase_.base().get(), ibase_.invPuncturedProdModBaseArray().get(),
             ibase_.baseProd().get(), obase_.base().get(), base_change_matrix_.get(), temp.get(), out.get());
            // auto p = temp.toHost();
        ...
        RNSToolCuda::RNSToolCuda(const RNSTool &copy)
            : coeff_count_(copy.coeff_count_),
              ...
              base_q_to_Bsk_conv_(
                  copy.base_q_to_Bsk_conv_.isNull() ? nullptr : new BaseConverterCuda(*copy.base_q_to_Bsk_conv_)),
              base_q_to_m_tilde_conv_(
                  copy.base_q_to_m_tilde_conv_.isNull() ? nullptr
                                                        : new BaseConverterCuda(*copy.base_q_to_m_tilde_conv_)),
              base_B_to_q_conv_(
                  copy.base_B_to_q_conv_.isNull() ? nullptr : new BaseConverterCuda(*copy.base_B_to_q_conv_)),
              base_B_to_m_sk_conv_(
                  copy.base_B_to_m_sk_conv_.isNull() ? nullptr : new BaseConverterCuda(*copy.base_B_to_m_sk_conv_)),
              base_q_to_t_gamma_conv_(
                  copy.base_q_to_t_gamma_conv_.isNull() ? nullptr
                                                        : new BaseConverterCuda(*copy.base_q_to_t_gamma_conv_)),
              base_q_to_t_conv_(
                  copy.base_q_to_t_conv_.isNull() ? nullptr : new BaseConverterCuda(*copy.base_q_to_t_conv_)),
              inv_prod_q_mod_Bsk_(copy.inv_prod_q_mod_Bsk_),
              neg_inv_prod_q_mod_m_tilde_(copy.neg_inv_prod_q_mod_m_tilde_),
              inv_prod_B_mod_m_sk_(copy.inv_prod_B_mod_m_sk_), inv_gamma_mod_t_(copy.inv_gamma_mod_t_),
              prod_B_mod_q_(copy.prod_B_mod_q_), inv_m_tilde_mod_Bsk_(copy.inv_m_tilde_mod_Bsk_),
              prod_q_mod_Bsk_(copy.prod_q_mod_Bsk_), neg_inv_q_mod_t_gamma_(copy.neg_inv_q_mod_t_gamma_),
              prod_t_gamma_mod_q_(copy.prod_t_gamma_mod_q_), inv_q_last_mod_q_(copy.inv_q_last_mod_q_),
              m_tilde_(copy.m_tilde_), m_sk_(copy.m_sk_), t_(copy.t_), gamma_(copy.gamma_),
              inv_q_last_mod_t_(copy.inv_q_last_mod_t_), q_last_mod_t_(copy.q_last_mod_t_)
        {
        ...
        __global__ void gDivideAndRoundqLastInplace(
            uint64_t *input, size_t coeff_count, size_t base_q_size, const Modulus *base_q,
            const MultiplyUIntModOperand *inv_q_last_mod_q, uint64_t half)
        {
            ...
            FOR_N(i, base_q_size - 1)
            {
                ...
                input[i * coeff_count + gindex] =
                    temp_result + (DeviceHelper::getModulusValue(b) & static_cast<std::uint64_t>(-borrow));
                input[i * coeff_count + gindex] =
                    kernel_util::dMultiplyUintMod(input[i * coeff_count + gindex], inv_q_last_mod_q[i], b);
            }
        }
        ...
        void RNSToolCuda::divideAndRoundqLastInplace(DevicePointer<uint64_t> input) const
        {
            ...
            (input.get(), coeff_count_, base_q_->size(), base_q_->base().get(), inv_q_last_mod_q_.get(), q_last_half_);
        }

        __global__ void gDivideAndRoundqLastNttInplaceStepA(
            uint64_t *last_input, uint64_t *temp, size_t coeff_count, size_t base_q_size, const Modulus *base_q,
            uint64_t half)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            last_input[gindex] = kernel_util::dAddUintMod(last_input[gindex], half, base_q[base_q_size - 1]);
        ...
        __global__ void gDivideAndRoundqLastNttInplaceStepB(
            const uint64_t *temp, uint64_t *input, size_t coeff_count, size_t base_q_size, const Modulus *base_q,
            ...
            DevicePointer<uint64_t> input, ConstDevicePointer<NTTTablesCuda> rns_ntt_tables) const
        {
            ...
            kernel_util::kInverseNttNegacyclicHarvey(
                input + coeff_count_ * (base_q_size - 1), 1, 1, getPowerOfTwo(coeff_count_),
                rns_ntt_tables + (base_q_size - 1));
            auto temp = temp_.ensure(coeff_count_ * (base_q_size - 1));
            KERNEL_CALL(gDivideAndRoundqLastNttInplaceStepA, coeff_count_)
            (input.get() + coeff_count_ * (base_q_size - 1), temp.get(), coeff_count_, base_q_size,
             base_q_->base().get(), q_last_half_);
            kernel_util::kNttNegacyclicHarveyLazy(
                temp.get(), 1, base_q_size - 1, getPowerOfTwo(coeff_count_), rns_ntt_tables);
            gDivideAndRoundqLastNttInplaceStepB<<<block_count, 256>>>(
                temp.get(), input.get(), coeff_count_, base_q_size, base_q_->base().get(), inv_q_last_mod_q_.get());
        }

        __global__ void gFaskbconvSk(
            const uint64_t *input, const uint64_t *temp, size_t coeff_count, const Modulus *m_sk, size_t base_B_size,
            size_t base_q_size, const Modulus *base_q, MultiplyUIntModOperand inv_prod_B_mod_m_sk,
        ...
        )
        {
            ...
            uint64_t alpha_sk = kernel_util::dMultiplyUintMod(
                temp[gindex] + (m_sk_value - input[base_B_size * coeff_count + gindex]), inv_prod_B_mod_m_sk, *m_sk);
            // printf("%llu\n", alpha_sk);
            ...
            FOR_N(i, base_q_size)
            {
                ...
                MultiplyUIntModOperand prod_B_mod_q_elt = kernel_util::dSetMultiplyModOperand(prod_B_mod_q[i], b);
                ...
                if (alpha_sk > m_sk_div_2)
                    ...
                    dest = kernel_util::dMultiplyAddUintMod(alpha_sk, neg_prod_B_mod_q_elt, dest, b);
            }
        }
        ...
        void RNSToolCuda::fastbconvSk(ConstDevicePointer<uint64_t> input, DevicePointer<uint64_t> destination) const
        {
            ...
            (input.get(), temp.get(), coeff_count_, m_sk_cuda_.get(), base_B_size, base_q_size, base_q_->base().get(),
             inv_prod_B_mod_m_sk_, prod_B_mod_q_.get(), destination.get());
        }

        __global__ void gSmMrq(
            const uint64_t *input, size_t coeff_count, size_t base_Bsk_size, const Modulus *base_Bsk,
            const uint64_t m_tilde_div_2, const Modulus *m_tilde, MultiplyUIntModOperand neg_inv_prod_q_mod_m_tilde,
            const uint64_t *prod_q_mod_Bsk, const MultiplyUIntModOperand *inv_m_tilde_mod_Bsk, uint64_t *destination)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            const uint64_t *input_m_tilde = input + base_Bsk_size * coeff_count;
            uint64_t r_m_tilde =
                kernel_util::dMultiplyUintMod(input_m_tilde[gindex], neg_inv_prod_q_mod_m_tilde, *m_tilde);
            FOR_N(i, base_Bsk_size)
            {
                const Modulus &b = base_Bsk[i];
                MultiplyUIntModOperand prod_q_mod_Bsk_elt = kernel_util::dSetMultiplyModOperand(prod_q_mod_Bsk[i], b);
        ...
        void RNSToolCuda::smMrq(ConstDevicePointer<uint64_t> input, DevicePointer<uint64_t> destination) const
        {
            ...
            (input.get(), coeff_count_, base_Bsk_size, base_Bsk_->base().get(), m_tilde_div_2, m_tilde_cuda_.get(),
             neg_inv_prod_q_mod_m_tilde_, prod_q_mod_Bsk_.get(), inv_m_tilde_mod_Bsk_.get(), destination.get());
        }

        __global__ void gFastFloor(
            const uint64_t *input, size_t coeff_count, size_t base_Bsk_size, const Modulus *base_Bsk,
            const MultiplyUIntModOperand *inv_prod_q_mod_Bsk, uint64_t *destination)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            FOR_N(i, base_Bsk_size)
            {
                ...
                destination[id] = kernel_util::dMultiplyUintMod(
                    input[id] + (DeviceHelper::getModulusValue(base_Bsk[i]) - destination[id]), inv_prod_q_mod_Bsk[i],
                    base_Bsk[i]);
            }
        }
        ...
        void RNSToolCuda::fastFloor(ConstDevicePointer<uint64_t> input, DevicePointer<uint64_t> destination) const
        {
            ...
            (input.get(), coeff_count_, base_Bsk_size, base_Bsk_->base().get(), inv_prod_q_mod_Bsk_.get(),
             destination.get());
        }
        ...
        void RNSToolCuda::fastbconvmTilde(ConstDevicePointer<uint64_t> input, DevicePointer<uint64_t> destination) const
        {
            ...
            kernel_util::kMultiplyPolyScalarCoeffmod(
                input, 1, base_q_size, coeff_count_, m_tilde_.value(), base_q_->base(), temp.get());
        ...
        __global__ void gDecryptScaleAndRoundStepA(
            const uint64_t *input, size_t base_q_size, const Modulus *base_q, size_t coeff_count,
            const MultiplyUIntModOperand *prod_t_gamma_mod_q, uint64_t *temp)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            FOR_N(i, base_q_size)
            {
        ...
        __global__ void gDecryptScaleAndRoundStepB(
            const uint64_t *temp_t_gamma, size_t coeff_count, uint64_t gamma, const Modulus *t,
            MultiplyUIntModOperand inv_gamma_mod_t, uint64_t *destination)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            uint64_t gamma_div_2 = gamma >> 1;
            if (temp_t_gamma[coeff_count + gindex] > gamma_div_2)
                destination[gindex] = kernel_util::dAddUintMod(
                    temp_t_gamma[gindex], kernel_util::dBarrettReduce64(gamma - temp_t_gamma[coeff_count + gindex], *t),
                    *t);
            else
                destination[gindex] = kernel_util::dSubUintMod(
                    temp_t_gamma[gindex], kernel_util::dBarrettReduce64(temp_t_gamma[coeff_count + gindex], *t), *t);
            if (0 != destination[gindex])
                destination[gindex] = kernel_util::dMultiplyUintMod(destination[gindex], inv_gamma_mod_t, *t);
        }

        void RNSToolCuda::decryptScaleAndRound(
            ConstDevicePointer<uint64_t> input, DevicePointer<uint64_t> destination) const
        {
            size_t base_q_size = base_q_->size();
            ...
            (input.get(), base_q_size, base_q_->base().get(), coeff_count_, prod_t_gamma_mod_q_.get(), temp.get());
            ...
            gDecryptScaleAndRoundStepA<<<block_count, 256>>>(
                temp_t_gamma.get(), base_t_gamma_size, base_t_gamma_->base().get(), coeff_count_,
                neg_inv_q_mod_t_gamma_.get(), temp_t_gamma.get());

            gDecryptScaleAndRoundStepB<<<block_count, 256>>>(
                temp_t_gamma.get(), coeff_count_, gamma_.value(), t_cuda_.get(), inv_gamma_mod_t_, destination.get());
        }

        __global__ void gModTAndDivideqLastInplace(
            uint64_t *input, size_t coeff_count, size_t modulus_size, const Modulus *base_q,
            const Modulus *plain_modulus, uint64_t inv_q_last_mod_t, const MultiplyUIntModOperand *inv_q_last_mod_q)
        {
            GET_INDEX_COND_RETURN(coeff_count);
            ...
            FOR_N(i, modulus_size - 1)
            {
                ...
                input[i * coeff_count + gindex] +=
                    two_times_q_i - kernel_util::dBarrettReduce64(last_input[gindex], base_q[i]) - delta_mod_q_i;
                input[i * coeff_count + gindex] =
                    kernel_util::dMultiplyUintMod(input[i * coeff_count + gindex], inv_q_last_mod_q[i], base_q[i]);
            }
        }
        ...
        void RNSToolCuda::modTAndDivideqLastInplace(DevicePointer<uint64_t> input) const
        {
            ...
            (input.get(), coeff_count_, modulus_size, base_q_->base().get(), t_cuda_.get(), inv_q_last_mod_t_,

    ...
    namespace util
    {
        ...
        public:
            RNSBaseCuda(const RNSBase &copy)
                : size_(copy.size_), base_(copy.base_), base_prod_(copy.base_prod_),
            ...
            ConstDevicePointer<Modulus> base() const
            {
                return base_.asPointer();
            }
        ...
        {
        public:
            BaseConverterCuda(const BaseConverter &copy) : ibase_(copy.ibase_), obase_(copy.obase_)
            {
                ...
                for (size_t i = 0; i < obase_.size(); i++)
                {
                    KernelProvider::copy<uint64_t>(
                        base_change_matrix_.get() + i * isize, copy.base_change_matrix_[i].get(), isize);
                }
            }

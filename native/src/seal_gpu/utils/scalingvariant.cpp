            ...
            for (size_t i = 0; i < plain_coeff_count; i++)
            {
                ...
                for (size_t j = 0; j < coeff_modulus_size; j++)
                {
                    uint64_t scaled_rounded_coeff =
                        multiplyAddUintMod(plain_data[i], coeff_div_plain_modulus[j], fix[0], coeff_modulus[j]);
                    // std::cout << destination[j * coeff_count + i] << std::endl;
                    destination[j * coeff_count + i] =
                        addUintMod(destination[j * coeff_count + i], scaled_rounded_coeff, coeff_modulus[j]);
            ...
            for (size_t i = 0; i < plain_coeff_count; i++)
            {
                ...
                for (size_t j = 0; j < coeff_modulus_size; j++)
                {
                    uint64_t scaled_rounded_coeff =
                        multiplyAddUintMod(plain_data[i], coeff_div_plain_modulus[j], fix[0], coeff_modulus[j]);
                    destination[j * coeff_count + i] =
                        subUintMod(destination[j * coeff_count + i], scaled_rounded_coeff, coeff_modulus[j]);
                }
            }
        }
    } // namespace util
